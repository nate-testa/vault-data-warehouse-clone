...
import time
import re
import majesco_data_feed_mapping
...
class MajescoDataFeedProcessor:
    def __init__(self, csv_filename, table_name):
        # File and directory configurations
        self.csv_filename = csv_filename
        self.table_name = table_name
        self.container_name = 'inbound-majesco-billing-data-feed'
        self.files_folder = 'Raw'
        self.archive_folder = 'archive'
        self.local_to_process_dir = HOME_PATH + '/airflow/tmp_files/majesco_data_feed/to_process'
        self.local_processed_dir = HOME_PATH + '/airflow/tmp_files/majesco_data_feed/processed_files'
    ...
    def read_and_prepare_data(self):
        try:
            ...
            expected_columns = majesco_data_feed_mapping.get_mapping(self.table_name)
            # Normalize expected column names to lowercase
            expected_columns_lower = {k.lower().strip(): v for k, v in expected_columns.items()}

            # Read all lines and extract the Generate Date from line 4
            with open(self.local_file_path, "r", encoding="utf-8-sig") as f:
                all_lines = f.readlines()
            
            # Extract Generate Date from line 4 and convert to SQL format
            generate_date_str = all_lines[3].split(',')[1].strip()
            generate_date_formatted = pd.to_datetime(generate_date_str).strftime('%Y-%m-%d')
            
            # Extract the CSV block (column names and data) from the file, starting from line 8
            csv_lines = all_lines[7:]
            ...
            # Add start_date and end_date with the extracted generate date
            df['start_date'] = generate_date_formatted
            df['end_date'] = generate_date_formatted
            
            # Add 'create_ts' with current date
            df['create_ts'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]

            # Reorder columns to match the SQL table
            sql_columns = list(expected_columns.values()) + ['start_date', 'end_date', 'create_ts']
            df = df[sql_columns]
...
def table_for_csv(file_name):
    ...
    for phrase, table in majesco_data_feed_mapping.PHRASE_TO_TABLE.items():
        if phrase in stem:
            return table
        ...
        with mssql_hook.get_conn() as connection:
            with connection.cursor() as cursor:
                query = """
                    ...
                    WHERE process_nm = 'py_majesco_data_feed'
                """
...
def process_all_files():
    try:
        ...
        delete_old_files(HOME_PATH + '/airflow/tmp_files/majesco_data_feed/to_process', retention_days=2)
        delete_old_files(HOME_PATH + '/airflow/tmp_files/majesco_data_feed/processed_files', retention_days=2)

        # Azure Blob Storage configuration
        wasb_conn_id = 'azure_blob_storage'
        container_name = 'inbound-majesco-billing-data-feed'
        files_folder = 'Raw'
        ...
        for blob_name, blob_last_modified in sorted_csv_files:
            ...
            try:
                ...
                etl_audit_sk = start_etl_process('py_majesco_data_feed', f'file_name: {csv_filename}')
                processor = MajescoDataFeedProcessor(csv_filename,table_name)
                processor.process()
                update_tetl_control_table(
                    process_nm='py_majesco_data_feed',
                ...
                if etl_audit_sk: # Log failure only if etl_audit_sk was created
...
if __name__ == "__main__":
    ...
    #**Manual Execution**
    processor = MajescoDataFeedProcessor('NOTES DATA FEED_202601200000010465.csv','stage_majesco_notes_data_feed')
    processor.process()
