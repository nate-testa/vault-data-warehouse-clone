...
from snapsheet_api_post import process_financial_transaction_action, financial_transaction_action_qry
...
def snapsheet_api_financial_transaction_action_send_email(**kwargs):
    ...
    msg_text = f"The following report provides the status of rows processed on [{current_date}] through the Snapsheet financial transaction action API."
    
    # if result is not None:
    EmailOperator(
        task_id='send_email_snapsheet',
        to=to_email,
        subject='Airflow - Snapsheet financial transaction action API',
...
with DAG(
    dag_id='Snapsheet_Financial_Transaction_Action_Daily_Feed',
    catchup=False,
    max_active_runs=1,
    default_args=args,
    ...
    snapsheet_items = [
            'sp_claim_financial_transaction_action_snapsheet_api'
        ]

    sp_claim_financial_transaction_action_snapsheet_api = MsSqlOperator(
            task_id='sp_claim_financial_transaction_action_snapsheet_api',
            mssql_conn_id='Vault_EDW',
            sql="EXEC edw_core.sp_claim_financial_transaction_action_snapsheet_api",
            database="vault_edw",
            autocommit=True,
        )
    ...
    send_snapsheet_email = EmailOperator(
            ...
            to=to_email,
            subject='Airflow - Snapsheet financial transaction action table loaded successfully',
            html_content=get_sp_success_data_HTML(snapsheet_items, 'All stored procedures executed successfully for all the Snapsheet financial transaction action tables'),
        )
    
    py_snapsheet_api_financial_transaction_action_send_email = PythonOperator(
            task_id='py_snapsheet_api_financial_transaction_action_send_email',
            python_callable=snapsheet_api_financial_transaction_action_send_email,
            provide_context=True,
            dag=dag,
        )
...
start >> sp_claim_financial_transaction_action_snapsheet_api >> py_process_financial_transaction_action >> send_snapsheet_email >> py_snapsheet_api_financial_transaction_action_send_email >> end
