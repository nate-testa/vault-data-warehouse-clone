...
def auto_executed_today(**kwargs):
    sql_qry = """
                SELECT process_nm, status_desc
                FROM edw_core.tetl_audit
                WHERE process_nm = 'sp_claim_clue_auto_feed'
...
with DAG(
    dag_id='vault_CLUE_auto_daily_feed',
    catchup=False,
    max_active_runs=1,
    default_args=args,
    ...
    clue_auto_executed_today = BranchPythonOperator(
        task_id='clue_auto_executed_today',
        python_callable=auto_executed_today,
        dag=dag,
    )
...
start >> clue_auto_executed_today >> [continue_task, skip_task] 
skip_task >> end
continue_task >> CLUE_Auto_group >> end