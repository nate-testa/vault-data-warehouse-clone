...
class LC360Processor:
    ...
    def create_sql_server_engine(self):
        mssql_hook = MsSqlHook(mssql_conn_id=self.mssql_conn_id)
        self.engine = mssql_hook.get_sqlalchemy_engine()
        self.connection = mssql_hook.get_conn()
    ...
    def insert_data_into_sql(self, df):
        try:
            ...
            # Insert data into SQL Server table 'edw_cat_model.lc360_temp_table'
            df.to_sql(
                ...
                index=False,
                chunksize=500,
                method='multi'  # Optional: speeds up the insertion
            )
    ...
    def process_sql_operations(self):
        try:
            # Establish a cursor
            cursor = self.connection.cursor()

            # Step 7: Insert data from temp table into 'Insp_LC360_historical'
            ...
            self.connection.commit()
            logging.info("Data inserted into 'Insp_LC360_historical' successfully.")

            # Step 8: Check data count in 'Insp_LC360_historical'
            ...
            # Step 9: Compare historical to cleaned data
            compare_sql = """
                WITH 
                historical AS (
                    SELECT inspection_update_dt, COUNT(*) AS historical
            ...
            # Step 10: Insert historical data into cleaned table
            insert_cleaned_sql = """
                WITH 
            ...
            self.connection.commit()
            logging.info("Data inserted into 'Insp_LC360_cleaned' successfully.")

            # Step 11: Sanity check changes in inspection status
            sanity_check_sql = """
                WITH 
                insp_new AS (
                    SELECT * 
            ...
            logging.error(f"Error during SQL operations: {e}")
            raise
    ...
    def process(self):
        try:
            ...
            self.insert_data_into_sql(df)
            self.delete_existing_records()
            self.process_sql_operations()
