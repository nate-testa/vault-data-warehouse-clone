--Update edw_core.tvalidation_sql, for...
--tclaim_feature snapsheet claims - missing vehicle_coverage_sk
--tclaim_feature snapsheet claims - missing item_sk
--tclaim_feature snapsheet claims - missing coverage_sk
...
source_sql =
'select count(*) from edw_core.tclaim c, edw_core.tclaim_feature cf where c.claim_sk = cf.claim_sk and cf.source_system_sk = 5 
...
update edw_core.tvalidation_sql
set target_sql = 'select 7',
source_sql =
'select count(*) from edw_core.tclaim c, edw_core.tclaim_feature cf where c.claim_sk = cf.claim_sk 
and cf.source_system_sk = 5 
and c.policy_no not in (''AU9999VES'', ''AU9999VRE'', ''COV9999VES'', ''COV9999VRE'', ''FPP9999VES'', ''FPP9999VRE'', ''FPP9998VRE'')
