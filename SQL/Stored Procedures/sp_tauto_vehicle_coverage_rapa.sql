...
-- Description: This stored procedure insert and update info related to tauto_vehicle_coverage_rapa.
--------------------------------------------------------------------------------------------------------------------------------------------------
...
CREATE OR ALTER PROCEDURE [edw_core].[sp_tauto_vehicle_coverage_rapa]
AS
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS [edw_temp].[tauto_vehicle_coverage_rapa_temp1];

		SELECT 
			IssuedDate, policy_no, effective_dt, vehicle_no, transaction_effective_dt, expiration_dt, transaction_dt, transaction_seq_no, policy_history_sk, auto_vehicle_sk, auto_vehicle_coverage_sk
        ...
        INTO [edw_temp].[tauto_vehicle_coverage_rapa_temp1]
        FROM
			(
                SELECT
                    acct.IssuedDate, acct.PolicyNumber as policy_no, acct.EffectiveDate as effective_dt, av.[vehicle_no] as vehicle_no, acctvo.[UniqueId] as vehicle_unique_id, acct.TransactionEffectiveDate as transaction_effective_dt, 
                    acct.ExpirationDate as expiration_dt, acct.IssuedDate as transaction_dt, acct.PolicyChangeNumber as transaction_seq_no,
                    ph.policy_history_sk, av.auto_vehicle_sk, avc.auto_vehicle_coverage_sk, 
				...
				INNER JOIN [edw_core].[tauto_vehicle_coverage] AS avc
					ON avc.policy_no = acct.PolicyNumber
                    AND avc.effective_dt = CAST(acct.EffectiveDate AS DATE)
                    AND avc.transaction_seq_no = acct.policychangenumber
					AND avc.vehicle_unique_id = av.vehicle_unique_id
		...
		INSERT INTO [edw_core].[tauto_vehicle_coverage_rapa]
        (
            policy_no,
            effective_dt,
            vehicle_no,
        ...
        FROM 
            [edw_temp].[tauto_vehicle_coverage_rapa_temp1] AS t1
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(IssuedDate) FROM edw_temp.[tauto_vehicle_coverage_rapa_temp1]),@last_source_extract_ts);
