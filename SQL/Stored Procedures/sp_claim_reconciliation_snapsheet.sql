...
-- ================================================================================================= 
CREATE OR ALTER PROCEDURE [edw_core].[sp_claim_reconciliation_snapsheet]
AS
BEGIN
	...
	BEGIN TRY		
		...
		drop table if exists edw_temp.loss_incurred_validation_snapsheet
        drop table if exists edw_temp.loss_incurred_validation_edw
        ...
        into edw_temp.loss_incurred_validation_snapsheet
        from
        (  select * from 
            (
        ...
        SUM(       ct.loss_reserve_amt+ct.expense_reserve_amt+ct.defense_reserve_amt   +ct.subrogation_recovery_reserve_amt+ct.salvage_recovery_reserve_amt+
                    ...
                    INTO edw_temp.loss_incurred_validation_edw
        ...
        SELECT c.claim_sk, cf.claim_feature_sk,b.claim_no,
        ...
        from
        (
        select claim_number, exposure_id,sum(total_loss_incurred) as total_loss_incurred from edw_temp.loss_incurred_validation_snapsheet --where claim_number='C21HOA00007'
        group by claim_number, exposure_id
        )a,
        edw_temp.loss_incurred_validation_edw b,
        edw_core.tclaim c,
