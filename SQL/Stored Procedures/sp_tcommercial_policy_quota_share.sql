...
-- Description: This stored procedure insert and update info related to tcommercial_policy_quota_share.
-----------------------------------------------------------------------------------------------------------------------
-- Change date          |Author						|	Change Description
-----------------------------------------------------------------------------------------------------------------------
-- 19/03/2025           Alberto Almario				1. Created this procedure 
-- ===================================================================================================================== 
CREATE OR ALTER PROCEDURE [edw_core].[sp_tcommercial_policy_quota_share]
AS
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS edw_temp.tcommercial_policy_quota_share_temp1;
		DROP TABLE IF EXISTS edw_temp.tcommercial_policy_quota_share_temp2;
		DROP TABLE IF EXISTS edw_temp.tcommercial_policy_quota_share_temp3;
		DROP TABLE IF EXISTS edw_temp.tcommercial_policy_quota_share_temp4;
		...
		SELECT 
			...
			,IssuedDate
		INTO edw_temp.tcommercial_policy_quota_share_temp1 
		...
		SELECT	
			...
			,IsDeletedOnPolicyChange as quota_share_deleted_in
		...
		INTO edw_temp.tcommercial_policy_quota_share_temp2
		FROM
			(
				SELECT  
				...
				FROM edw_temp.tcommercial_policy_quota_share_temp1 acc
		...
		INTO edw_temp.tcommercial_policy_quota_share_temp3
		FROM edw_temp.tcommercial_policy_quota_share_temp1 acc
		INNER JOIN edw_stage.AccountTransactionVersion acctv ON acctv.AccountTransactionId = acc.Id
		INNER JOIN edw_stage.AccountTransactionVersionObject acctvo ON acctvo.AccountTransactionVersionId = acctv.Id
		...
		SELECT 
			...
			,cpt.commercial_policy_tower_sk
			...
			,tmp2.quota_share_deleted_in
			,GETDATE() as create_ts
			,GETDATE() as update_ts
			,@etl_audit_sk as etl_audit_sk
			,tmp1.source_system_sk
		INTO edw_temp.tcommercial_policy_quota_share_temp4
		FROM edw_temp.tcommercial_policy_quota_share_temp1 tmp1
		INNER JOIN edw_temp.tcommercial_policy_quota_share_temp2 tmp2 on tmp2.AccountTransactionId = tmp1.Id
		LEFT JOIN edw_temp.tcommercial_policy_quota_share_temp3 tmp3 on tmp3.AccountTransactionId = tmp1.Id and tmp3.ObjectId = tmp2.ParentObjectId
		...
		LEFT JOIN edw_commercial.tcommercial_policy_tower cpt on tmp1.PolicyNumber = cpt.policy_no and cast(tmp1.EffectiveDate as date) = cast(cpt.effective_dt as date) and tmp1.PolicyChangeNumber = cpt.transaction_seq_no and tmp3.tower_unique_id = cpt.tower_unique_id

		-- Insert process
		INSERT INTO edw_commercial.tcommercial_policy_quota_share
		 (
			 policy_no
			,effective_dt
			,expiration_dt
		...
		SELECT
			...
			,etl_audit_sk
			,source_system_sk
		FROM edw_temp.tcommercial_policy_quota_share_temp4	
		;
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(IssuedDate) FROM edw_temp.[tcommercial_policy_quota_share_temp1]),@last_source_extract_ts);
