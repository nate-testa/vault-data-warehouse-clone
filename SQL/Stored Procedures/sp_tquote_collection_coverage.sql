...
-- 01/22/25		Alberto Almario				    4. Added new column theft_or_loss_general_conditions_endorsement_in
-- ======================================================================================================== 
...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS [edw_temp].[tquote_collection_coverage_temp1];
		...
		SET @sql ='SELECT Id, PolicyNumber as quote_no, EffectiveDate, ExpirationDate, [Number]
			...
			FROM
			(
				SELECT acct.Id, acc.PolicyNumber, acc.EffectiveDate, acc.ExpirationDate, acc.[Number]
				...
				FROM
					...
					WHERE
						...
						AND acct.CreatedDate > @last_source_extract_ts --20230717 added
					) acc
		...
		INSERT INTO [edw_core].[tquote_collection_coverage] (
			[quote_no]
           ,[effective_dt]
           ,[expiration_dt]
           ,[transaction_seq_no]
		...
		SELECT 
            ...
            [quote_collection_location_sk],[quote_history_sk],[UnoccupiedMoreThanThreeMonths],[NumberOfLossesLastThreeYears],[ProtectionClass],
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(t1.createddate) FROM [edw_temp].[tquote_collection_coverage_temp1] t1),@last_source_extract_ts);
