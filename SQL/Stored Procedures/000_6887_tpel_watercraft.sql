--tpel_vehicle
select top 100 * from edw_core.tetl_audit where process_nm like '%tpel_watercraft%' order by etl_audit_sk desc;
SELECT * FROM edw_core.tetl_control where process_nm in ('sp_tpel_watercraft');
select watercraft_deleted_in, COUNT(1) from [edw_core].[tpel_watercraft] group by watercraft_deleted_in;
select * from [edw_core].[tpel_watercraft];

-- update edw_core.tetl_control set last_source_extract_ts = '1900-01-01 00:00:00' where process_nm in ('sp_tpel_watercraft');
-- truncate table [edw_core].[tpel_watercraft];
-- EXEC [edw_core].[sp_tpel_watercraft];

select count(1) from edw_integration.policy_ivans_pel_feed;
-- select * into [edw_temp].[policy_ivans_pel_feed_bk_20240815] from [edw_integration].[policy_ivans_pel_feed];
-- update edw_core.tetl_control set last_source_extract_ts = '1900-01-01 00:00:00' where process_nm in ('sp_policy_ivans_pel_feed');
