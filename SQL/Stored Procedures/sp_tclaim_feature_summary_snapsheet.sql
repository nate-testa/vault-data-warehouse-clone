...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			feature_closed_with_pay_ct=(CASE WHEN feature_closed_ct=1 AND (itd_total_paid_amt-itd_salvage_recovery_amt)>0 THEN 1 ELSE 0 END),
			feature_closed_without_pay_ct=(CASE WHEN feature_closed_ct=1 AND (itd_total_paid_amt-itd_salvage_recovery_amt)=0 THEN 1 ELSE 0 END)
