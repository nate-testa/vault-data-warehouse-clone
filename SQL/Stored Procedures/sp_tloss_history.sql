...
-- 07-30-2025				Dinesh Bobbili				3. Added logic to remove time from LossDate
-- ================================================================================================= 
CREATE OR ALTER PROCEDURE [edw_core].[sp_tloss_history]
AS
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS [edw_temp].[tloss_history_temp1];
		SELECT 
		...
		INTO [edw_temp].[tloss_history_temp1]
		FROM
			(
			SELECT
		...
		INSERT INTO [edw_core].[tloss_history] (
			[policy_no]
			,[effective_dt]
		...
		SELECT 
			...
			END
		FROM 
			[edw_temp].[tloss_history_temp1]

		SET @rows_affected=@@ROWCOUNT;

		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(t1.IssuedDate) FROM edw_temp.[tloss_history_temp1] t1),@last_source_extract_ts);
