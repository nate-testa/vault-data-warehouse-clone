...
-- Description: This procedures inserts task data for commercial policies
----------------------------------------------------------------------------------------------------------------------
-- Change date      |Author			    |	Change Description
----------------------------------------------------------------------------------------------------------------------
-- 03/31/24		    Yunus Mohammed		1. Created this procedure 
-- ==================================================================================================================== 

CREATE or ALTER PROCEDURE [edw_core].[sp_tcommercial_task]
...
BEGIN 
	...
	BEGIN TRY
        ...
        SELECT 
			...
			, cust.customer_sk
			, wt.id as task_id
        INTO edw_temp.tcommercial_temp1 
		...
		MERGE edw_commercial.tcommercial_task AS Target
		USING 
		(	
			 SELECT 	policy_no, effective_dt, transaction_effective_dt, transaction_seq_no, 
				...
				FROM edw_temp.tcommercial_temp1
		)  AS Source
		...
		VALUES (Source.policy_no, Source.effective_dt, Source.transaction_effective_dt, Source.transaction_seq_no, 
                        ...
                        Source.created_by_sk, Source.assigned_to_sk, Source.completed_by_sk, source.customer_sk, source.task_id,
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(GREATEST(t1.task_created_dt,t1.task_updated_dt)) FROM edw_temp.tcommercial_temp1 t1),@last_source_extract_ts)
