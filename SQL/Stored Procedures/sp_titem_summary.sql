/****** Object:  StoredProcedure [edw_core].[sp_titem_summary]    Script Date: 2/7/2024 4:55:42 PM ******/
SET ANSI_NULLS ON
GO
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN 
				...
				--insert cancels
				 ...
				 into edw_temp.titem_summary_can_rein_temp1
				...
				delete a
				from edw_temp.titem_summary_can_rein_temp1 a
				inner join ( select policy_sk, item_sk, 
							...
							from edw_temp.titem_summary_can_rein_temp1
							group by policy_sk, item_sk) b on a.policy_sk = b.policy_sk and a.item_sk = b.item_sk
				where b.cancel_ct = b.rein_ct;	

				 
				DROP TABLE IF EXISTS edw_temp.titem_summary_max_tr;
				select policy_sk, item_sk, max(transaction_seq_no) transaction_seq_no
				into edw_temp.titem_summary_max_tr
				from edw_core.tpolicy_transaction 
				where isnull(item_sk,0) <> 0 
				...
				inner join (select policy_sk, item_sk 
							from edw_temp.titem_summary_can_rein_temp1 
				            group by policy_sk, item_sk
							having count(distinct policy_transaction_type_sk) > 1) c on a.policy_sk = c.policy_sk and a.item_sk = c.item_sk
				where not exists (select * from edw_temp.titem_summary_max_tr b 
								  where a.policy_sk = b.policy_sk and a.item_sk = b.item_sk and a.transaction_seq_no = b.transaction_seq_no);
				...
				with inf as
				(
				 SELECT policy_sk, item_sk, coverage_sk, vehicle_coverage_sk,
				...
				min_tr as
				(
					select distinct policy_sk,item_sk,  
							min(calendar_month_sk) over (partition by policy_sk,item_sk --order by transaction_seq_no
								) calendar_month_sk,
							max(transaction_seq_no) over (partition by policy_sk,item_sk) max_tr_seq_no
					from edw_core.tpolicy_transaction 
					where isnull(item_sk,0) <> 0 
				 	and  effective_dt_sk <= @end_dt_sk
					and   transaction_effective_dt_sk <= @end_dt_sk
				...
				xpsr_new as
				( 
				 SELECT tr.policy_sk,tr.item_sk,
						(
							case
				 ...
				 left join edw_core.titem_inforce inf on inf.policy_sk = tr.policy_sk and inf.item_sk = tr.item_sk and inf.month_sk = @end_dt_sk
				 where isnull(tr.item_sk,0) <> 0 
				 ...
				 and   tr.policy_transaction_sk = (select min(tr2.policy_transaction_sk) from edw_core.tpolicy_transaction tr2 
				 								   where tr2.policy_sk = tr.policy_sk and tr2.item_sk = tr.item_sk and tr2.transaction_seq_no = tr.transaction_seq_no) 
				),
				xpsr_exp as
				( 
				 SELECT inf.policy_sk, inf.item_sk,
						0 we,
				...
				xpsr_cancel as
				( 
					 select policy_sk , item_sk, sum(we) we, sum(ee) ee
					 from
					 (
						 SELECT distinct tr.policy_sk, tr.item_sk, tr.transaction_seq_no,
		 						(
						 ...
						 FROM edw_core.tpolicy_transaction tr
						 inner join edw_core.tpolicy pol on tr.policy_sk = pol.policy_sk
						 left join min_tr on min_tr.policy_sk = tr.policy_sk and min_tr.item_sk = tr.item_sk  
						 --inner join min_tr max_tr on max_tr.policy_sk = tr.policy_sk and max_tr.max_tr_seq_no = tr.transaction_seq_no  
						 where isnull(tr.item_sk,0) <> 0 
				 		 and  exists (select policy_sk from edw_temp.titem_summary_can_rein_temp1 c 
				...
				( 
					 ...
					 from
					 (
						 SELECT distinct tr.policy_sk, tr.item_sk, transaction_seq_no,
		 						(
						 ...
						 --inner join min_tr max_tr on max_tr.policy_sk = tr.policy_sk and max_tr.max_tr_seq_no = tr.transaction_seq_no 
						 where isnull(tr.item_sk,0) <> 0 
				 		and  exists (select policy_sk from edw_temp.titem_summary_can_rein_temp1 r 
