	...
	BEGIN TRY
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN 
				...
				with inf as
				(
				 SELECT policy_sk, item_sk, coverage_sk, 
				...
				( 
				 ...
				 and   tr.transaction_seq_no = (select min(tr1.transaction_seq_no) from edw_core.tpolicy_transaction tr1 where tr1.policy_sk = tr.policy_sk and tr1.item_sk = tr.item_sk)
				 and   tr.policy_transaction_sk = (select min(tr2.policy_transaction_sk) from edw_core.tpolicy_transaction tr2 where tr2.policy_sk = tr.policy_sk and tr2.item_sk = tr.item_sk and tr2.transaction_seq_no = tr.transaction_seq_no) 
				),
				xpsr_exp as
				( 
				 SELECT pol.policy_sk, inf.item_sk,
						0 we,
				...
				( 
				 ...
				 and   exists (select policy_sk from cancels c where tr.policy_sk = c.policy_sk and tr.item_sk = c.item_sk) 
				 and   tr.transaction_seq_no = (select max(tr1.transaction_seq_no) from edw_core.tpolicy_transaction tr1 where tr1.policy_sk = tr.policy_sk and tr1.item_sk = tr.item_sk)
				 and   tr.policy_transaction_sk = (select max(tr2.policy_transaction_sk) from edw_core.tpolicy_transaction tr2 where tr2.policy_sk = tr.policy_sk and tr2.item_sk = tr.item_sk and tr2.transaction_seq_no = tr.transaction_seq_no)
				),
				xpsr_rein as
				( 
				 SELECT tr.policy_sk, tr.item_sk, 
				 ...
				 FROM edw_core.tpolicy_transaction tr, edw_core.tpolicy pol 
				 where tr.policy_sk = pol.policy_sk
				 and   exists (select policy_sk from rein r where tr.policy_sk = r.policy_sk and tr.item_sk = r.item_sk)
				 and   tr.transaction_seq_no = (select max(tr1.transaction_seq_no) from edw_core.tpolicy_transaction tr1 where tr1.policy_sk = tr.policy_sk and tr1.item_sk = tr.item_sk)
				 and   tr.policy_transaction_sk = (select max(tr2.policy_transaction_sk) from edw_core.tpolicy_transaction tr2 where tr2.policy_sk = tr.policy_sk and tr2.item_sk = tr.item_sk and tr2.transaction_seq_no = tr.transaction_seq_no)
				),
				prm as
				...
				INSERT INTO edw_core.titem_summary
					( 
						month_sk, policy_sk, item_sk, coverage_sk, 
