-- =================================================================================================
-- Description: This procedures inserts commercial claim transactio data
-----------------------------------------------------------------------------------------------------------
-- Change date          |Author						         |	Change Description
-----------------------------------------------------------------------------------------------------------
-- 11/18/25		         Yunus Mohammed			1. Created this procedure
-- ======================================================================================================== 
CREATE OR ALTER PROCEDURE [edw_core].[sp_tcommercial_claim_transaction]
AS
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS edw_temp.tcommercial_claim_transaction_temp1;
		DROP TABLE IF EXISTS edw_temp.tcommercial_claim_transaction_temp2;
		DROP TABLE IF EXISTS edw_temp.tcommercial_claim_transaction_temp3;
		DROP TABLE IF EXISTS edw_temp.tcommercial_claim_transaction_temp4;
		DROP TABLE IF EXISTS edw_temp.tcommercial_claim_transaction_temp5;
		...
		INTO edw_temp.tcommercial_claim_transaction_temp1
		...
		INNER JOIN edw_core.tcommercial_claim tc ON tc.claim_no = c.claim_number
		LEFT JOIN edw_core.tcommercial_claim_feature tf ON tf.claim_no = tc.claim_no and e.id = tf.claim_coverage_cd
		...
		SELECT
			 a.commercial_claim_sk 
			,a.commercial_claim_feature_sk
			,a.product_sk 
		...
		LEFT JOIN edw_commercial.tcommercial_claim_transaction_type ctt on a.claim_transaction_type_cd = ctt.claim_transaction_type_cd
...
/* Payment query will automatically excludes migrated payments becuase they are not loaded into financial_transaction_actions */    
		SELECT 
            ...
            ,tf.commercial_claim_feature_sk
			,tc.product_sk 
			,tc.commercialpolicy_sk 
			,tb.broker_sk 
            ,tcu.customer_sk
			,cp.commercial_claim_payment_sk
			,c.claim_number
			,e.exposure_name
			,e.coverage_premium_class
			,e.coverage_name
		...
		INNER JOIN edw_commercial.tcommercial_claim_feature tf ON tf.claim_no = tc.claim_no and e.id = tf.claim_coverage_cd
		LEFT JOIN edw_commercial.tcommercial_policy tp ON tp.policy_sk = tc.policy_sk
		LEFT JOIN edw_core.tbroker tb ON tb.broker_id = tp.broker_id
		LEFT JOIN edw_core.tcustomer tcu ON tcu.customer_id = tp.customer_id
		LEFT JOIN edw_core.tdate as td1 ON td1.actual_dt = CAST(fta.created_at AS DATE)
		LEFT JOIN edw_core.tclaim_cost_category as tcc on tcc.claim_cost_category_nm = res.cost_category
...
SELECT 
			...
			,pay.cost_category,
            case 
				when pay.cost_type like '%_claim%' 		and res.reserve_method is NULL 			then 'claim'
				when pay.cost_type like '%_adjusting%' 	and res.reserve_method is NULL 			then 'adjusting'
				when pay.cost_type like '%_defense%' 	and res.reserve_method is NULL 			then 'defense'
				when pay.cost_type like '%_claim%' 		and res.reserve_method = 'subrogation' 	then 'claim-subrogation'
				when pay.cost_type like '%_claim%' 		and res.reserve_method = 'salvage' 		then 'claim-salvage'
				when pay.cost_type like '%_claim%' 		and res.reserve_method = 'overpayment' 	then 'claim-overpayment'
				when pay.cost_type like '%_claim%' 		and res.reserve_method = 'deductible' 	then 'claim-deductible'
				when pay.cost_type like '%_claim%' 		and res.reserve_method = 'reinsurance' 	then 'claim-reinsurance'
				when pay.cost_type like '%_adjusting%' 	and res.reserve_method = 'salvage'  	then 'adjusting-salvage'
				when pay.cost_type like '%_adjusting%' 	and res.reserve_method = 'subrogation'  then 'adjusting-subrogation'
				when pay.cost_type like '%_adjusting%' 	and res.reserve_method = 'overpayment'  then 'adjusting-overpayment'
				when pay.cost_type like '%_adjusting%' 	and res.reserve_method = 'deductible'   then 'adjusting-deductible'
				when pay.cost_type like '%_adjusting%' 	and res.reserve_method = 'reinsurance'  then 'adjusting-reinsurance'
				when pay.cost_type like '%_defense%' 	and res.reserve_method = 'salvage' 		then 'defense-salvage'
				when pay.cost_type like '%_defense%' 	and res.reserve_method = 'subrogation' 	then 'defense-subrogation'
				when pay.cost_type like '%_defense%' 	and res.reserve_method = 'overpayment' 	then 'defense-overpayment'
				when pay.cost_type like '%_defense%' 	and res.reserve_method = 'deductible' 	then 'defense-deductible'
				when pay.cost_type like '%_defense%' 	and res.reserve_method = 'reinsurance' 	then 'defense-reinsurance'
			end as claim_transaction_type_cd,
            5 as source_system_sk ,
            NULL AS feature_status_sk,
            tcc.claim_cost_category_sk,
        ...
        LEFT JOIN edw_stage_snapsheet.financial_reserve_items res ON pay.financial_transaction_id = res.financial_transaction_id AND pay.cost_type = res.cost_type AND pay.exposure_id = res.exposure_id AND pay.cost_category = res.cost_category
		...
		INNER JOIN edw_stage_snapsheet.claims c on c.id = pay.claim_id
		INNER JOIN edw_stage_snapsheet.exposures e on e.claim_id = pay.claim_id and pay.exposure_id=e.id
		INNER JOIN edw_commercial.tcommercial_claim tc ON tc.claim_no = c.claim_number
		INNER JOIN edw_commercial.tcommercial_claim_feature tf ON tf.claim_no = tc.claim_no and e.id = tf.claim_coverage_cd
		LEFT JOIN edw_commercial.tcommercial_policy tp ON tp.commercial_policy_sk = tc.pcommercial_olicy_sk
		LEFT JOIN edw_core.tbroker tb ON tb.broker_id = tp.broker_id
		LEFT JOIN edw_core.tcustomer tcu ON tcu.customer_id = tp.customer_id
		LEFT JOIN edw_core.tdate as td1 ON td1.actual_dt = CAST(fta.created_at AS DATE)
		LEFT JOIN edw_core.tclaim_cost_category as tcc on tcc.claim_cost_category_nm = pay.cost_category
		LEFT JOIN edw_commercial.tcommercial_claim_payment as cp on  cp.commercial_claim_feature_sk=tf.commercial_claim_feature_sk 
                and cp.settle_payee_id=ft.remote_identifier --> join on ebao payment column
     	WHERE 1=1
		...
		INTO edw_temp.tcommercial_claim_transaction_temp5
		FROM edw_temp.tcommercial_claim_transaction_temp2 a
		LEFT JOIN edw_temp.tcommercial_claim_transaction_temp3 b
			ON a.commercial_claim_sk = b.commercial_claim_sk
       ...
       SELECT 
			...
			b.reinsurance_defense_recovery_amt,
			b.overpayment_defense_recovery_amt 
       FROM edw_temp.tcommercial_claim_transaction_temp4 b LEFT JOIN edw_core.tclaim_transaction_type ctt on b.claim_transaction_type_cd = ctt.claim_transaction_type_cd
       ;
	...
	-- Start Insert process
		...
		FROM edw_temp.tcommercial_claim_transaction_temp5;
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(created_at) FROM edw_temp.tcommercial_claim_transaction_temp5),@last_source_extract_ts);
