...
-- Description: This procedures insert Participant Personal Excess Liability driver data
-- =============================================
---------------------------------------------------------------------------------------------------
...
-- ================================================================================================= 
CREATE OR ALTER PROCEDURE [edw_core].[sp_tgrpel_driver]
...
BEGIN
	...
	BEGIN TRY
		...
		drop table if exists edw_temp.tgrpel_driver_temp1
		select 
			PolicyNumber,EffectiveDate,ExpirationDate,TransactionEffectiveDate,TransactionDate,transaction_seq_no,
            policy_history_sk,source_system_sk,IssuedDate,rownum as [Index],
			...
			driver_unique_id			
	    into edw_temp.tgrpel_driver_temp1
		from
		(
		select * 
		from
		...
		INSERT INTO [edw_core].[tgrpel_driver]
		(
		...
		FROM
			edw_temp.tgrpel_driver_temp1 AS ttlc

		SET @rows_affected=@@ROWCOUNT;

		-- Update control table
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(IssuedDate) FROM edw_temp.tgrpel_driver_temp1),@last_source_extract_ts);	
