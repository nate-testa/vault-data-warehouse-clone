...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS edw_temp.tgrpel_master_coverage_enrollment_temp1;
		
		select
            acc.PolicyNumber as grpel_master_policy_no,acc.EffectiveDate as effective_dt, acc.ExpirationDate as expiration_dt,
            ...
            aes.CreatedDate
        into edw_temp.tgrpel_master_coverage_enrollment_temp1
        from
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(t.CreatedDate) FROM edw_temp.tgrpel_master_coverage_enrollment_temp1 t),@last_source_extract_ts);
