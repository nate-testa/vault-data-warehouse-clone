...
BEGIN 
	...
	BEGIN TRY
        ...
        DROP TABLE IF EXISTS edw_temp.tpolicy_insured_temp1
        SELECT
			acctr.*,
			case when acctr.ExternalSourceId is not NULL then 2--(AV2) 
				 Else 4 --(Metal)
			end ssk
		INTO edw_temp.tpolicy_insured_temp1
		...
		-- Pivot Table
		DROP TABLE IF EXISTS edw_temp.tpolicy_insured_temp2; 
		...
		FROM
			(
				SELECT  acctv.AccountTransactionId, versionobjectid,
						acctvof.Field, 
						acctvof.Value
				FROM edw_temp.tpolicy_insured_temp1 acc
		...
		LEFT JOIN edw_core.tpolicy_history ph on ph.Policy_no = pol.policy_no and ph.effective_dt = pol.effective_dt and ph.transaction_seq_no = t1.PolicyChangeNumber 
		;
