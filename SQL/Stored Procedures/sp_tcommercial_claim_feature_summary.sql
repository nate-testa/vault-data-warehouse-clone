...
-- Description: This procedure calculate commercial claim amounts at coverage level
---------------------------------------------------------------------------------------------------
-- Change date          |Author						                    |	Change Description
---------------------------------------------------------------------------------------------------
-- 11/27/2025		  Yunus Mohammed					1. Procedure created
-- ================================================================================================= 
   
CREATE OR ALTER PROCEDURE [edw_core].[sp_tcommercial_claim_feature_summary]
AS
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			DELETE FROM edw_commercial.tcommercial_claim_feature_summary WHERE month_sk=@end_dt_sk
			INSERT INTO edw_commercial.tcommercial_claim_feature_summary
			(
				month_sk,commercial_claim_sk,commercial_claim_feature_sk,product_sk,commercial_policy_sk,broker_sk,customer_sk,
			...
			SELECT
			@end_dt_sk AS month_sk, ct.commercial_claim_sk,ct.commercial_claim_feature_sk,c.product_sk,c.commercial_policy_sk, ct.broker_sk,ct.customer_sk,
			...
			FROM
			edw_commercial.tcommercial_claim_transaction ct
			INNER JOIN edw_commercial.tcommercial_claim c ON ct.commercial_claim_sk =c.commercial_claim_sk
			INNER JOIN edw_commercial.tcommercial_claim_feature cf ON ct.commercial_claim_feature_sk =cf.commercial_claim_feature_sk 
			INNER JOIN
			(
				SELECT *
				FROM
				(
					SELECT ROW_NUMBER() OVER (PARTITION BY commercial_claim_feature_sk ORDER BY transaction_ts DESC) AS rn, 
					tt.commercial_claim_feature_sk, tt.feature_status_sk
					FROM edw_commercial.tcommercial_claim_transaction tt
					WHERE tt.transaction_dt_sk <= @end_dt_sk
				)b
				WHERE rn=1
			) AS fs ON ct.commercial_claim_feature_sk =fs.commercial_claim_feature_sk
			...
			GROUP BY ct.commercial_claim_sk,ct.commercial_claim_feature_sk,cf.aslob_sk,c.product_sk,c.commercial_policy_sk, 
            ct.broker_sk,ct.customer_sk,c.source_system_sk;
		
			SET @rows_affected=@@ROWCOUNT
		
			UPDATE edw_commercial.tcommercial_claim_feature_summary
			SET
