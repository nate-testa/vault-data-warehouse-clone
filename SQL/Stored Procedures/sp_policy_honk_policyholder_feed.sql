...
BEGIN
	...
	BEGIN TRY
		...
		from (select distinct pol.policy_no as policy_number,
					REPLACE(pins.first_nm, '"', '')  as first_name,
					REPLACE(case when pins.insured_type = 'Entity' then isnull(pins.insured_nm, 'Unknown') 
					else isnull(pins.last_nm, isnull(pins.insured_nm, 'Unknown')) end, '"', '') as last_name,
					REPLACE(pins.mailing_address_line_1, '"', '') as address,
					REPLACE(pins.mailing_address_city_nm, '"', '') as city,
					REPLACE(pins.mailing_address_state_cd, '"', '') as state,
