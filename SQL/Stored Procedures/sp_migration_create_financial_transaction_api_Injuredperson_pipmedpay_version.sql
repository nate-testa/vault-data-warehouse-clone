 ...
 --               else 'cancelled' --added on 01172025 cause some claims initial payment transaction status has been updated to cancel when actual cancellation transaction came in--
           ...
           AS [data.attributes.stage],
            ...
            CASE
                ...
                (
                        resh.reserve_type='RC_01' AND settle.claim_type LIKE '%SAL%' AND 
						CAST(SETTLE_payee.payee_name AS VARCHAR(MAX))='Copart'
                ) THEN settle_changed
				WHEN (resh.reserve_type='RC_01' AND settle.claim_type='LOS' AND settle_changed < 0 )
					OR (resh.reserve_type ='RC_01' AND settle.claim_type LIKE '%SAL%' AND CAST(settle_payee.payee_name AS VARCHAR(MAX)) NOT IN ('Copart')) THEN settle_changed
				WHEN resh.reserve_type='RC_02' AND 
					(
						(settle.claim_type = 'LOS' AND settle_changed < 0)
						OR
						(settle.claim_type ='LOS,SAL,SUB'  AND settle_changed != 0 )
					) THEN settle_changed
            END AS paid_amt,
            CASE
