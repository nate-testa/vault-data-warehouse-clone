...
-- Description: This procedures insert updates policy referral message
------------------------------------------------------------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS edw_temp.tpolicy_referral_message_temp1
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(GREATEST(referral_message_created_ts,referral_message_updated_ts)) 
                        FROM edw_temp.tpolicy_referral_message_temp1),@last_source_extract_ts)	
