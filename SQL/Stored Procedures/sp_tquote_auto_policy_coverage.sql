...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS [edw_temp].[tquote_auto_policy_coverage_temp1];

		SELECT 
			CreatedDate, quote_no, effective_dt, expiration_dt, transaction_seq_no, quote_history_sk,
        ...
        SELECT 
            ...
            ,t1.[ConsentToRateOTCCOLL] as consent_to_rate_otccoll
        FROM 
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(CreatedDate) FROM edw_temp.[tquote_auto_policy_coverage_temp1]),@last_source_extract_ts);
