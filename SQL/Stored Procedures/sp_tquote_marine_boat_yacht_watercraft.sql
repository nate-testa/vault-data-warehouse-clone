...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS edw_temp.tquote_marine_boat_yacht_watercraft_temp1;
		...
		SELECT 
			...
			source_system_sk,
			CreatedDate
		INTO edw_temp.tquote_marine_boat_yacht_watercraft_temp1
		FROM
			(
				SELECT
		...
		INSERT INTO [edw_core].[tquote_marine_boat_yacht_watercraft]
		(
			quote_no,
			effective_dt,
			expiration_dt,
		...
		FROM edw_temp.tquote_marine_boat_yacht_watercraft_temp1;
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(CreatedDate) FROM edw_temp.tquote_marine_boat_yacht_watercraft_temp1),@last_source_extract_ts)	
