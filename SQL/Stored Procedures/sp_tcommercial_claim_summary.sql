...
-- Description: This procedure calculate commercial claim amounts at claim level
---------------------------------------------------------------------------------------------------
-- Change date                |Author						                |	Change Description
---------------------------------------------------------------------------------------------------
-- 11/27/2025		        Yunus Mohammed					1. Procedure created
-- ================================================================================================= 
CREATE OR ALTER PROCEDURE [edw_core].[sp_tcommercial_claim_summary]
AS
BEGIN
DECLARE @ProcedureName NVARCHAR(120)
	...
	BEGIN TRY
		...
		BEGIN
			...
			DELETE FROM edw_commercial.tcommercial_claim_summary WHERE month_sk=@end_dt_sk

			INSERT INTO edw_commercial.tcommercial_claim_summary
			(
				month_sk,commercial_claim_sk,product_sk,commercial_policy_sk,broker_sk,customer_sk,
			...
			SELECT
			@end_dt_sk AS month_sk, commercial_claim_sk,product_sk,commercial_policy_sk,broker_sk,customer_sk,
			...
			FROM
				edw_commercial.tcommercial_claim_feature_summary 
			WHERE month_sk=@end_dt_sk
			GROUP BY month_sk, commercial_claim_sk, product_sk, commercial_policy_sk, broker_sk, customer_sk,source_system_sk
		
			SET @rows_affected=@@ROWCOUNT

			UPDATE edw_commercial.tcommercial_claim_summary
			SET
