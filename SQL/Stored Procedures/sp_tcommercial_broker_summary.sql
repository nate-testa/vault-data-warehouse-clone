		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				(
					select br.broker_sk, 
							-- 
							count(distinct case when CAST(q.quote_create_ts AS DATE) between 		 @prior_year_begin_dt and @prior_year_month_end_dt then q.commercial_quote_sk end) prior_ytd_submission_ct, 
							count(distinct case when CAST(q.quote_create_ts AS DATE) between 		 	   @year_begin_dt and 				   @end_dt then q.commercial_quote_sk end) 	  ytd_submission_ct, 
							count(distinct case when CAST(q.quote_create_ts AS DATE) between 	  @prior_quarter_begin_dt and @prior_year_month_end_dt then q.commercial_quote_sk end) prior_qtd_submission_ct, 
							count(distinct case when CAST(q.quote_create_ts AS DATE) between 		 	@quarter_begin_dt and 				   @end_dt then q.commercial_quote_sk end) 	  qtd_submission_ct, 
							count(distinct case when CAST(q.quote_create_ts AS DATE) between @prior_year_month_begin_dt and @prior_year_month_end_dt then q.commercial_quote_sk end) prior_mtd_submission_ct, 
							count(distinct case when CAST(q.quote_create_ts AS DATE) between 		 	  @month_begin_dt and 				   @end_dt then q.commercial_quote_sk end) 	  mtd_submission_ct, 
							--
							...
							--
							count(case when q.bind_dt between 		 @prior_year_begin_dt and @prior_year_month_end_dt 	then q.commercial_quote_sk end) prior_ytd_bind_ct, 
							count(case when q.bind_dt between 		 	   @year_begin_dt and 				   @end_dt 	then q.commercial_quote_sk end) 	   ytd_bind_ct, 
							count(case when q.bind_dt between 	  @prior_quarter_begin_dt and @prior_year_month_end_dt 	then q.commercial_quote_sk end) prior_qtd_bind_ct, 
							count(case when q.bind_dt between 		 	@quarter_begin_dt and 				   @end_dt 	then q.commercial_quote_sk end) 	   qtd_bind_ct, 
							count(case when q.bind_dt between    @prior_year_month_begin_dt and @prior_year_month_end_dt 	then q.commercial_quote_sk end) prior_mtd_bind_ct, 
							count(case when q.bind_dt between 			  @month_begin_dt and 				   @end_dt 	then q.commercial_quote_sk end)	   mtd_bind_ct
							--
				...
				where      prior_ytd_submission_ct <> 0 or ytd_submission_ct <> 0 
						...
						or prior_ytd_quote_premium_amt <> 0 or ytd_quote_premium_amt <> 0 
						or prior_qtd_quote_premium_amt <> 0 or qtd_quote_premium_amt <> 0 
						or prior_mtd_quote_premium_amt <> 0 or mtd_quote_premium_amt <> 0 
				...
				SELECT summ.broker_sk,   
							...
							sum(case when summ.month_sk = @month_end_dt_sk 			  and summ.inforce_ct = 1 then summ.inforce_net_premium_amt else 0 end) inforce_premium_amt,
							...
							--
							count(distinct case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between    	  @prior_year_begin_dt and @prior_year_month_end_dt and summ.inforce_ct = 1 then pol.commercial_policy_sk end) prior_ytd_new_business_ct ,
							count(distinct case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between 	  	    @year_begin_dt and 			     	@end_dt and summ.inforce_ct = 1 then pol.commercial_policy_sk end) 	   ytd_new_business_ct ,
							count(distinct case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between    @prior_quarter_begin_dt and @prior_year_month_end_dt and summ.inforce_ct = 1 then pol.commercial_policy_sk end) prior_qtd_new_business_ct ,
							count(distinct case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between 	  	 @quarter_begin_dt and 				 	@end_dt and summ.inforce_ct = 1 then pol.commercial_policy_sk end) 	   qtd_new_business_ct ,
							...
							sum(           case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between 	  		@year_begin_dt and 			   	 	@end_dt then summ.mtd_net_premium_amt else 0 end) 	     ytd_new_business_premium_amt ,   
							sum(           case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between    @prior_quarter_begin_dt and @prior_year_month_end_dt then summ.mtd_net_premium_amt else 0 end) prior_qtd_new_business_premium_amt ,   
							sum(           case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between 	  	 @quarter_begin_dt and 				 	@end_dt then summ.mtd_net_premium_amt else 0 end) 	     qtd_new_business_premium_amt ,   
							sum(           case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between @prior_year_month_begin_dt and @prior_year_month_end_dt then summ.mtd_net_premium_amt else 0 end) prior_mtd_new_business_premium_amt ,   
							sum(           case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between 	  	   @month_begin_dt and 				 	@end_dt then summ.mtd_net_premium_amt else 0 end) 	     mtd_new_business_premium_amt ,  */
							--
							--
							sum(           case when pol.policy_term = 'New' and td.actual_dt between 	  @prior_year_begin_dt and @prior_year_month_end_dt then summ.mtd_net_premium_amt else 0 end) prior_ytd_new_business_premium_amt ,   
							sum(           case when pol.policy_term = 'New' and td.actual_dt between 	  		@year_begin_dt and 			   	 	@month_end_dt then summ.mtd_net_premium_amt else 0 end) 	     ytd_new_business_premium_amt ,   
							sum(           case when pol.policy_term = 'New' and td.actual_dt between    @prior_quarter_begin_dt and @prior_year_month_end_dt then summ.mtd_net_premium_amt else 0 end) prior_qtd_new_business_premium_amt ,   
							sum(           case when pol.policy_term = 'New' and td.actual_dt between 	  	 @quarter_begin_dt and 				 	@month_end_dt then summ.mtd_net_premium_amt else 0 end) 	     qtd_new_business_premium_amt ,   
							sum(           case when pol.policy_term = 'New' and td.actual_dt between @prior_year_month_begin_dt and @prior_year_month_end_dt then summ.mtd_net_premium_amt else 0 end) prior_mtd_new_business_premium_amt ,   
							sum(           case when pol.policy_term = 'New' and td.actual_dt between 	  	   @month_begin_dt and 				 	@month_end_dt then summ.mtd_net_premium_amt else 0 end) 	     mtd_new_business_premium_amt ,  
							--
							--sum(           case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between @prior_year_month_begin_dt and @prior_year_month_end_dt and summ.inforce_ct = 1 then ph.net_premium_amt else 0 end) prior_mtd_gross_new_business_premium_amt,   
							--sum(           case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between 	  	   @month_begin_dt and 				 	@end_dt and summ.inforce_ct = 1 then ph.net_premium_amt else 0 end) 	  mtd_gross_new_business_premium_amt,   
							--
							...
							--
							sum(case when  summ.month_sk >       @prior_year_month_end_dt_sk then summ.earned_net_premium_amt else 0 end)   one_year_earned_premium_amt,
							sum(case when  summ.month_sk > @prior_year_three_month_end_dt_sk then summ.earned_net_premium_amt else 0 end) three_year_earned_premium_amt, 
							...
							--
							sum(case when  summ.month_sk >  @prior_year_two_month_end_dt_sk then summ.earned_net_premium_amt else 0 end)  two_year_earned_premium_amt,
							sum(case when  summ.month_sk > @prior_year_five_month_end_dt_sk then summ.earned_net_premium_amt else 0 end) five_year_earned_premium_amt
							-- 
				 into edw_temp.tcommercial_broker_summ_pols
				 FROM	edw_commercial.tcommercial_policy_summary summ 
				 inner join edw_commercial.tcommercial_policy pol on summ.commercial_policy_sk = pol.commercial_policy_sk 
				...
				SELECT summ.broker_sk,   
				 ...
				 FROM	edw_commercial.tcommercial_policy_summary summ, edw_commercial.tcommercial_policy pol,   edw_core.tdate td,
				 		edw_commercial.tcommercial_policy_history ph,  
				 		(select commercial_policy_sk, min(transaction_seq_no) transaction_seq_no
							from edw_commercial.tcommercial_policy_history
							group by commercial_policy_sk ) min_ph
					 where	summ.month_sk between  @prior_year_five_month_end_dt_sk and @month_end_dt_sk
					 and td.date_sk = summ.month_sk
					 and 	summ.commercial_policy_sk = pol.commercial_policy_sk 
					 and 	summ.commercial_policy_sk = ph.commercial_policy_sk 
					 and 	ph.commercial_policy_sk = min_ph.commercial_policy_sk and ph.transaction_seq_no = min_ph.transaction_seq_no
					 group by summ.broker_sk 
					union all
					SELECT summ.broker_sk,  
							...
							-- 
							sum(case when  summ.month_sk >  @prior_year_two_month_end_dt_sk then summ.earned_premium_amt else 0 end)   two_year_earned_net_premium_amt,
							sum(case when  summ.month_sk > @prior_year_five_month_end_dt_sk then summ.earned_premium_amt else 0 end) five_year_earned_net_premium_am
							--
				 FROM	edw_commercial.tcommercial_policy_summary summ, edw_commercial.tcommercial_policy pol,   edw_core.tdate td,
				 		edw_commercial.tcommercial_policy_history ph,
				 		(select commercial_policy_sk, min(transaction_seq_no) transaction_seq_no
				...
				SELECT ph.broker_sk,   
							--
							sum(case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between @prior_year_month_begin_dt and @prior_year_month_end_dt and prior_inf.commercial_policy_sk is not null then ph.net_premium_amt else 0 end) prior_mtd_gross_new_business_premium_amt,   
							sum(case when pol.policy_term = 'New' and greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between 	  	    @month_begin_dt and 			 	 @end_dt and curr_inf.commercial_policy_sk is not null then ph.net_premium_amt else 0 end) 	  mtd_gross_new_business_premium_amt, 
							--
							sum(case when greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between @prior_year_month_begin_dt and @prior_year_month_end_dt and prior_inf.commercial_policy_sk is not null then ph.net_premium_amt else 0 end) prior_mtd_gross_net_premium_amt,   
							sum(case when greatest(cast(ph.transaction_ts as date), ph.transaction_effective_dt) between 	  	    @month_begin_dt and 			 	 @end_dt and curr_inf.commercial_policy_sk is not null then ph.net_premium_amt else 0 end) 	  mtd_gross_net_premium_amt 
							--
				into edw_temp.tcommercial_broker_summ_pols1
				FROM	edw_commercial.tcommercial_policy pol
				inner join  edw_commercial.tcommercial_policy_history ph on pol.commercial_policy_sk = ph.commercial_policy_sk
				inner join (select commercial_policy_sk, min(transaction_seq_no) transaction_seq_no
							from edw_commercial.tcommercial_policy_history
							where greatest(cast(transaction_ts as date), transaction_effective_dt) <= @end_dt 
							group by commercial_policy_sk ) min_ph on ph.commercial_policy_sk = min_ph.commercial_policy_sk and ph.transaction_seq_no = min_ph.transaction_seq_no
				left join edw_commercial.tcommercial_daily_inforce_policy prior_inf on pol.commercial_policy_sk = prior_inf.commercial_policy_sk and prior_inf.inforce_dt_sk = @prior_year_month_end_dt_sk 
				left join edw_commercial.tcommercial_daily_inforce_policy curr_inf  on pol.commercial_policy_sk =  curr_inf.commercial_policy_sk and  curr_inf.inforce_dt_sk = 		   @end_dt_sk  
				...
				select 	COALESCE(ps.broker_sk, 		ps1.broker_sk, 		ps2.broker_sk, 		cs.broker_sk, 	q.broker_sk) broker_sk, 
						...
						--
						sum(isnull(q.prior_ytd_quote_premium_amt,0)) prior_ytd_quote_premium_amt,
						sum(isnull(q.ytd_quote_premium_amt,0)) ytd_quote_premium_amt,  
						sum(isnull(q.prior_qtd_quote_premium_amt,0)) prior_qtd_quote_premium_amt,
						sum(isnull(q.qtd_quote_premium_amt,0)) qtd_quote_premium_amt,  
						sum(isnull(q.prior_mtd_quote_premium_amt,0)) prior_mtd_quote_premium_amt,
						sum(isnull(q.mtd_quote_premium_amt,0)) mtd_quote_premium_amt,  
						--
						sum(isnull(q.prior_ytd_quote_attachment_amt,0)) prior_ytd_quote_attachment_amt,
						sum(isnull(q.ytd_quote_attachment_amt,0)) 		  	   ytd_quote_attachment_amt,  
						sum(isnull(q.prior_qtd_quote_attachment_amt,0)) prior_qtd_quote_attachment_amt,
						sum(isnull(q.qtd_quote_attachment_amt,0)) 		  	   qtd_quote_attachment_amt,  
						sum(isnull(q.prior_mtd_quote_attachment_amt,0)) prior_mtd_quote_attachment_amt,
						sum(isnull(q.mtd_quote_attachment_amt,0)) 		  	   mtd_quote_attachment_amt,  
						--
						sum(isnull(q.prior_ytd_quote_limit_amt,0)) prior_ytd_quote_limit_amt,
						sum(isnull(q.ytd_quote_limit_amt,0)) 		  	  ytd_quote_limit_amt,  
						sum(isnull(q.prior_qtd_quote_limit_amt,0)) prior_qtd_quote_limit_amt,
						sum(isnull(q.qtd_quote_limit_amt,0)) 		  	  qtd_quote_limit_amt,  
						sum(isnull(q.prior_mtd_quote_limit_amt,0)) prior_mtd_quote_limit_amt,
						sum(isnull(q.mtd_quote_limit_amt,0)) 		  	  mtd_quote_limit_amt, 
						--
						...
						sum(isnull(ps.prior_ytd_new_business_premium_amt,0)) prior_ytd_new_business_premium_amt, 
						sum(isnull(ps.prior_qtd_new_business_ct,0)) prior_qtd_new_business_ct, 
						sum(isnull(ps.prior_qtd_new_business_premium_amt,0)) prior_qtd_new_business_premium_amt, 
						sum(isnull(ps.prior_mtd_new_business_ct,0)) prior_mtd_new_business_ct, 
						sum(isnull(ps.prior_mtd_new_business_premium_amt,0)) prior_mtd_new_business_premium_amt, 
						--
						sum(isnull(ps.ytd_new_business_ct,0)) ytd_new_business_ct, 
						sum(isnull(ps.ytd_new_business_premium_amt,0)) ytd_new_business_premium_amt, 
						sum(isnull(ps.qtd_new_business_ct,0)) qtd_new_business_ct, 
						sum(isnull(ps.qtd_new_business_premium_amt,0)) qtd_new_business_premium_amt, 
						sum(isnull(ps.mtd_new_business_ct,0)) mtd_new_business_ct, 
						sum(isnull(ps.mtd_new_business_premium_amt,0)) mtd_new_business_premium_amt, 
						--
						sum(isnull(ps.prior_ytd_new_business_attachment_amt,0)) prior_ytd_new_business_attachment_amt, 
						sum(isnull(ps.prior_qtd_new_business_attachment_amt,0)) prior_qtd_new_business_attachment_amt, 
						sum(isnull(ps.prior_mtd_new_business_attachment_amt,0)) prior_mtd_new_business_attachment_amt, 
						--
						sum(isnull(ps.ytd_new_business_attachment_amt,0)) ytd_new_business_attachment_amt, 
						sum(isnull(ps.qtd_new_business_attachment_amt,0)) qtd_new_business_attachment_amt, 
						sum(isnull(ps.mtd_new_business_attachment_amt,0)) mtd_new_business_attachment_amt, 
						--
						sum(isnull(ps.prior_ytd_new_business_limit_amt,0)) prior_ytd_new_business_limit_amt, 
						sum(isnull(ps.prior_qtd_new_business_limit_amt,0)) prior_qtd_new_business_limit_amt, 
						sum(isnull(ps.prior_mtd_new_business_limit_amt,0)) prior_mtd_new_business_limit_amt, 
						--
						sum(isnull(ps.ytd_new_business_limit_amt,0)) ytd_new_business_limit_amt, 
						sum(isnull(ps.qtd_new_business_limit_amt,0)) qtd_new_business_limit_amt, 
						sum(isnull(ps.mtd_new_business_limit_amt,0)) mtd_new_business_limit_amt, 
						--
						sum(isnull(ps1.prior_mtd_gross_new_business_premium_amt,0)) prior_mtd_gross_new_business_premium_amt, 
						sum(isnull(ps1.mtd_gross_new_business_premium_amt,0)) mtd_gross_new_business_premium_amt,
						--
						...
						sum(isnull(ps.prior_inforce_premium_amt,0)) prior_inforce_premium_amt,
						sum(isnull(ps.inforce_ct,0)) inforce_ct, 
						sum(isnull(ps.inforce_premium_amt,0)) inforce_premium_amt,  
						-- 
						...
						--
						sum(isnull(ps.inforce_attachment_amt,0)) inforce_attachment_amt, 
						sum(isnull(ps.inforce_limit_amt,0)) inforce_limit_amt, 
						--
						sum(isnull(ps.one_year_earned_premium_amt,0)) one_year_earned_premium_amt, 
						sum(isnull(ps.three_year_earned_premium_amt,0)) three_year_earned_premium_amt, 
						...
						-- 
						sum(isnull(ps.two_year_earned_premium_amt,0)) two_year_earned_premium_amt, 
						sum(isnull(ps.five_year_earned_premium_amt,0)) five_year_earned_premium_amt,   
						--     
				...
				from pol_summ ps
				full join pol_summ1 ps1 on ps.broker_sk = ps1.broker_sk  
				full join pol_summ2 ps2 on ps.broker_sk = ps2.broker_sk 
				full join clm_summ cs  on ps.broker_sk =  cs.broker_sk 
				full join quotes q     on ps.broker_sk =   q.broker_sk  
				...
				INSERT INTO edw_commercial.tcommercial_broker_summary
					( 
						month_sk,
						broker_sk, 
						--
						...
						mtd_quote_ct,
						--
						prior_ytd_quote_premium_amt,
						ytd_quote_premium_amt,
						prior_qtd_quote_premium_amt,
						qtd_quote_premium_amt,
						prior_mtd_quote_premium_amt,
						mtd_quote_premium_amt,
						--
						prior_ytd_quote_attachment_amt,
						ytd_quote_attachment_amt,
						prior_qtd_quote_attachment_amt,
						qtd_quote_attachment_amt,
						prior_mtd_quote_attachment_amt,
						mtd_quote_attachment_amt,
						--
						prior_ytd_quote_limit_amt,
						ytd_quote_limit_amt,
						prior_qtd_quote_limit_amt,
						qtd_quote_limit_amt,
						prior_mtd_quote_limit_amt,
						mtd_quote_limit_amt,
						--
						prior_customer_ct,
						customer_ct,
						...
						prior_ytd_new_business_premium_amt, 
						prior_qtd_new_business_ct, 
						prior_qtd_new_business_premium_amt, 
						prior_mtd_new_business_ct, 
						prior_mtd_new_business_premium_amt, 
						--
						ytd_new_business_ct, 
						ytd_new_business_premium_amt,  
						qtd_new_business_ct, 
						qtd_new_business_premium_amt,  
						mtd_new_business_ct, 
						mtd_new_business_premium_amt, 
						--
						prior_ytd_new_business_attachment_amt, 
						prior_qtd_new_business_attachment_amt, 
						prior_mtd_new_business_attachment_amt, 
						--
						ytd_new_business_attachment_amt, 
						qtd_new_business_attachment_amt, 
						mtd_new_business_attachment_amt, 
						--
						prior_ytd_new_business_limit_amt, 
						prior_qtd_new_business_limit_amt, 
						prior_mtd_new_business_limit_amt, 
						--
						ytd_new_business_limit_amt, 
						qtd_new_business_limit_amt, 
						mtd_new_business_limit_amt, 
						--
						prior_mtd_gross_new_business_premium_amt, 
						mtd_gross_new_business_premium_amt,
						--
						--prior_mtd_premium_amt, 
						--mtd_premium_amt, 
						--
						...
						--
						prior_inforce_ct, 
						prior_inforce_premium_amt,
						inforce_ct, 
						inforce_premium_amt,  
						-- 
						mtd_mid_term_cancellation_ct, 
						--
						inforce_attachment_amt, 
						inforce_limit_amt, 
						--
						one_year_earned_premium_amt, 
						three_year_earned_premium_amt, 
						one_year_earned_exposure, 
						three_year_earned_exposure,
						--
						two_year_earned_premium_amt, 
						five_year_earned_premium_amt,  
						--
						one_year_claim_ct, 
						...
						policy_renewal_offered_premium_amt,
						...
						ytd_policy_renewal_offered_premium_amt,
				...
				select @month_end_dt_sk, 
						...
						--
						sum(summ.prior_ytd_quote_premium_amt), 
						sum(summ.ytd_quote_premium_amt),
						sum(summ.prior_qtd_quote_premium_amt), 
						sum(summ.qtd_quote_premium_amt),
						sum(summ.prior_mtd_quote_premium_amt), 
						sum(summ.mtd_quote_premium_amt),  
						--
						sum(summ.prior_ytd_quote_attachment_amt), 
						sum(summ.ytd_quote_attachment_amt),
						sum(summ.prior_qtd_quote_attachment_amt), 
						sum(summ.qtd_quote_attachment_amt),
						sum(summ.prior_mtd_quote_attachment_amt), 
						sum(summ.mtd_quote_attachment_amt), 
						--
						sum(summ.prior_ytd_quote_limit_amt), 
						sum(summ.ytd_quote_limit_amt),
						sum(summ.prior_qtd_quote_limit_amt), 
						sum(summ.qtd_quote_limit_amt),
						sum(summ.prior_mtd_quote_limit_amt), 
						sum(summ.mtd_quote_limit_amt),   
						--
						...
						sum(summ.prior_ytd_new_business_premium_amt), 
						sum(summ.prior_qtd_new_business_ct), 
						sum(summ.prior_qtd_new_business_premium_amt), 
						sum(summ.prior_mtd_new_business_ct), 
						sum(summ.prior_mtd_new_business_premium_amt), 
						--
						sum(summ.ytd_new_business_ct), 
						sum(summ.ytd_new_business_premium_amt), 
						sum(summ.qtd_new_business_ct), 
						sum(summ.qtd_new_business_premium_amt), 
						sum(summ.mtd_new_business_ct), 
						sum(summ.mtd_new_business_premium_amt), 
						--
						sum(summ.prior_ytd_new_business_attachment_amt),  
						sum(summ.prior_qtd_new_business_attachment_amt),  
						sum(summ.prior_mtd_new_business_attachment_amt),  
						--
						sum(summ.ytd_new_business_attachment_amt),  
						sum(summ.qtd_new_business_attachment_amt),  
						sum(summ.mtd_new_business_attachment_amt),  
						--
						sum(summ.prior_ytd_new_business_limit_amt),  
						sum(summ.prior_qtd_new_business_limit_amt),  
						sum(summ.prior_mtd_new_business_limit_amt),  
						--
						sum(summ.ytd_new_business_limit_amt),  
						sum(summ.qtd_new_business_limit_amt),  
						sum(summ.mtd_new_business_limit_amt),  
						--
						sum(summ.prior_mtd_gross_new_business_premium_amt) prior_mtd_gross_new_business_premium_amt, 
						sum(summ.mtd_gross_new_business_premium_amt) mtd_gross_new_business_premium_amt, 
						--
						--sum(summ.prior_mtd_gross_net_premium_amt) prior_mtd_premium_amt, 
						...
						--
						sum(summ.prior_inforce_ct), sum(summ.prior_inforce_premium_amt),  
						sum(summ.inforce_ct), sum(summ.inforce_premium_amt),  
						--
						sum(summ.mtd_mt_cancels),  
						--
						sum(summ.inforce_attachment_amt), sum(summ.inforce_limit_amt),   
						--
						sum(summ.one_year_earned_premium_amt), 
						sum(summ.three_year_earned_premium_amt), 
						sum(summ.one_year_earned_exposure), 
						sum(summ.three_year_earned_exposure), 
						--
						sum(summ.two_year_earned_premium_amt), 
						sum(summ.five_year_earned_premium_amt), 
						--
				...
				UPDATE edw_commercial.tcommercial_broker_summary
				set				
						one_year_loss_ratio  				= case when [one_year_earned_premium_amt]			= 0 then 0 else 100.0*[one_year_loss_incurred_amt]/[one_year_earned_premium_amt] end,
						one_year_loss_ratio_capped 			= case when [one_year_earned_premium_amt]			= 0 then 0 else 100.0*[one_year_loss_incurred_capped_amt]/[one_year_earned_premium_amt] end,
						...
						--
						three_year_loss_ratio 				= case when [three_year_earned_premium_amt]			= 0 then 0 else 100.0*[three_year_loss_incurred_amt]/[three_year_earned_premium_amt] end,
						three_year_loss_ratio_capped 		= case when [three_year_earned_premium_amt]			= 0 then 0 else 100.0*[three_year_loss_incurred_capped_amt]/[three_year_earned_premium_amt] end, 
