...
-- Description: This procedures loads vendor reports Clue_Auto data
---------------------------------------------------------------------------------------------------
-- Change date |Author						|	Change Description
---------------------------------------------------------------------------------------------------
-- 16/07/24		Hernando Gonzalez			1. Created this procedure  
-- ================================================================================================= 
CREATE OR ALTER PROCEDURE [edw_core].[sp_tvendor_report_clue_auto]
AS
BEGIN
	...
	BEGIN TRY
        ...
        DROP TABLE IF EXISTS [edw_temp].[tvendor_report_clue_auto_temp1];
        SELECT 
            acc.policynumber, 
            acc.effectivedate, 
            accr.dateordered, 
        ...
        INTO [edw_temp].[tvendor_report_clue_auto_temp1]
        ...
        AND accr.source = 'Clue Auto'
        AND GREATEST(accri.UpdatedDate,accri.CreatedDate) > @last_source_extract_ts
        ;


        INSERT INTO [edw_stage].[tvendor_report_clue_auto]
        SELECT 
            vrh.policynumber, 
            vrh.effectivedate, 
            vrh.dateordered, 
            ...
            (
                SELECT 
                    ...
                    vrhj.[Label],
                    vrhj.[Value] 
                FROM [edw_temp].[tvendor_report_clue_auto_temp1] AS vrhj
                WHERE 1=1
        ...
        FROM [edw_temp].[tvendor_report_clue_auto_temp1] AS vrh
        GROUP BY 
            vrh.policynumber, 
            vrh.effectivedate, 
            vrh.dateordered, 
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(UpdatedDate) FROM edw_temp.[tvendor_report_clue_auto_temp1]),@last_source_extract_ts);
