	...
	BEGIN TRY
		...
		(
			...
			(
				select  accr.source, accr.reporttype, accri.Category, accri.[Group], 
						replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(label,
						',',''),
		...
		select @sql='insert into ' 
					...
					+ ' select	 acc.policynumber, acc.effectivedate, 
												...
												case when accri.Category = accri.[Group] or accri.Label = accri.[Group] then concat(accri.Category, '' - '',replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(accri.label,''Could not get over to dock. Odd situation. Their dock is on the neighbors side due to the neighbors tram to his dock. With lake so low, could not navigate the loose rocks in my business attire. '',''Could not get to dock as its on the neighbors side. ''),''Water Leak Detection Alarm Systems'',''Water Leak Detectn Alarm Sys''),''Could not climb back to this unit without crushing duct. Access is from the other scuttle I could not get into.'',''Cant climb back to unit without crushing duct. Access is from other scuttle I could not get into''),''Activate Leak Defense Automatic Water Shut Off Device (if the occupancy of the house is not primary)'',''Activate Leak Defense Auto Water Shut Off Device (if the occupancy of the house is not primary)''),'' Toilet Supply Lines That Have Plastic B-nut Connectors and'','' Toilet Lines having Plastic B-nut Conn and''),''when construction starts on second floor'',''construction on 2nd fl''),''Bar sink cabinet next to icemaker. Water in and under cab. Client called plumber while I was there.'',''Bar sink cab next to icemaker. Water in under cab. Client called plumber while I was there.''),''['','' - ''),'']'',''''),'''''''',''''))
													 when accri.Category <> accri.[Group] then concat(accri.Category, '' - '', accri.[Group], '' - '',replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(accri.label,''Could not get over to dock. Odd situation. Their dock is on the neighbors side due to the neighbors tram to his dock. With lake so low, could not navigate the loose rocks in my business attire. '',''Could not get to dock as its on the neighbors side. ''),''Water Leak Detection Alarm Systems'',''Water Leak Detectn Alarm Sys''),''Could not climb back to this unit without crushing duct. Access is from the other scuttle I could not get into.'',''Cant climb back to unit without crushing duct. Access is from other scuttle I could not get into''),''Activate Leak Defense Automatic Water Shut Off Device (if the occupancy of the house is not primary)'',''Activate Leak Defense Auto Water Shut Off Device (if the occupancy of the house is not primary)''),'' Toilet Supply Lines That Have Plastic B-nut Connectors and'','' Toilet Lines having Plastic B-nut Conn and''),''when construction starts on second floor'',''construction on 2nd fl''),''Bar sink cabinet next to icemaker. Water in and under cab. Client called plumber while I was there.'',''Bar sink cab next to icemaker. Water in under cab. Client called plumber while I was there.''),''['','' - ''),'']'',''''),'''''''',''''))
													 else ''''
