...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS edw_temp.tcommercial_policy_tower_temp1;
		DROP TABLE IF EXISTS edw_temp.tcommercial_policy_tower_temp2;
		...
		SELECT 
			...
			,IssuedDate
		INTO edw_temp.tcommercial_policy_tower_temp1 
		...
		INTO edw_temp.tcommercial_policy_tower_temp2
		FROM
			(
				SELECT  
					...
					,acctvof.Field
					,acctvof.Value
				FROM edw_temp.tcommercial_policy_tower_temp1 acc
		...
		-- Insert process
		INSERT INTO edw_commercial.tcommercial_policy_tower
		(
			 policy_no
			,effective_dt
			,expiration_dt
		...
		SELECT 
			...
			,tmp2.tower_deleted_in
			,GETDATE() as create_ts
			,GETDATE() as update_ts
			,@etl_audit_sk as etl_audit_sk
			,tmp1.source_system_sk
			,tmp2.tower_no
		FROM edw_temp.tcommercial_policy_tower_temp1 tmp1
		LEFT JOIN edw_temp.tcommercial_policy_tower_temp2 tmp2 on tmp2.AccountTransactionId = tmp1.Id
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(IssuedDate) FROM edw_temp.[tcommercial_policy_tower_temp1]),@last_source_extract_ts);
