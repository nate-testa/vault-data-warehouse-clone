...
BEGIN 
	...
	BEGIN TRY
		...
		INTO edw_temp.tquote_insured_wip_temp1
		...
		INTO edw_temp.tquote_insured_wip_temp2
		FROM
			(
				SELECT  accvo.AccountId, accvof.ObjectId,
						accvof.Field, 
						accvof.Value
				FROM edw_temp.tquote_insured_wip_temp1 acc
				...
				WHERE COALESCE(LTRIM(RTRIM(accvof.Field)), '''') != ''''
				and (accvo.objecttype = 'Insured'
				)
			) t
		PIVOT 
			(
		...
		select 	t1.PolicyNumber, t1.EffectiveDate, 0 AS [Number], qh.quote_history_sk, 
		...
		FROM
            edw_temp.tquote_insured_wip_temp1 t1
		    INNER JOIN edw_temp.tquote_insured_wip_temp2 t2 on t1.id = t2.AccountId
		    LEFT JOIN edw_core.tquote_history qh on qh.quote_no = t1.PolicyNumber and qh.effective_dt = t1.EffectiveDate and qh.transaction_seq_no = 0
		;
