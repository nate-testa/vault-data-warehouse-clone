...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS [edw_temp].[tquote_collection_location_temp1];
		SELECT 
			Id, PolicyNumber as quote_no, EffectiveDate, CreatedDate, UpdatedDate, AccountTransactionId,  [NamedInsured], [CompanyName], [RiskAddressLine1], [RiskAddressLine2], [UnitFloor], [RiskAddressCity], [RiskAddressState], [RiskAddressZipCode]
			,[RiskAddressCounty], [RiskAddressCountry]
			,[Latitude], [Longitude]
			,source_system_sk --20230717 added
		INTO [edw_temp].[tquote_collection_location_temp1]
		FROM
			(
			SELECT
				acct.Id, acc.PolicyNumber, acc.EffectiveDate, acc.CreatedDate, acc.UpdatedDate, acctv.AccountTransactionId, accto.[Field], accto.[Value]
		...
		VALUES (Source.quote_no, Source.EffectiveDate, Source.[address_line_1], Source.[address_line_2], 
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(t1.createddate) FROM edw_temp.[tquote_collection_location_temp1] t1),@last_source_extract_ts);
