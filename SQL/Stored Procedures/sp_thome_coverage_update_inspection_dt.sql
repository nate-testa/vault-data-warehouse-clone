...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF exists edw_temp.thome_coverage_update_inspection_dt_temp1; 

		DROP TABLE IF exists edw_temp.thome_coverage_update_inspection_dt_temp1_final; 
		...
		into   	edw_temp.thome_coverage_update_inspection_dt_temp1
		...
		into  edw_temp.thome_coverage_update_inspection_dt_temp1_final
		from   edw_temp.thome_coverage_update_inspection_dt_temp1 a
		...
		insert into  edw_temp.thome_coverage_update_inspection_dt_temp1_final
		...
		inner join edw_temp.thome_coverage_update_inspection_dt_temp1_final a  on a.original_policy_no = pol.original_policy_no and pol.term_no > a.term_no
		where  	pol.policy_no not in (select policynumber from edw_temp.thome_coverage_update_inspection_dt_temp1_final) 
		...
		inner join 	edw_temp.thome_coverage_update_inspection_dt_temp1_final a on cov.policy_no = a.policynumber  ;
		...
		DROP TABLE IF exists edw_temp.thome_coverage_update_inspection_dt_temp3; 
		DROP TABLE IF exists edw_temp.thome_coverage_update_inspection_dt_temp2; 

		--pull all policies that were cancel rewritten
		select policy_no, effective_dt, max(last_inspection_dt) last_inspection_dt
		into edw_temp.thome_coverage_update_inspection_dt_temp3
		...
		inner join edw_temp.thome_coverage_update_inspection_dt_temp3 a on pol.prior_policy_no = a.policy_no 
		...
		into edw_temp.thome_coverage_update_inspection_dt_temp2
		from	 edw_core.tpolicy pol 
		...
		set cov.last_inspection_dt = a.last_inspection_dt 
		from  edw_core.thome_coverage cov 
		inner join edw_temp.thome_coverage_update_inspection_dt_temp2 a on cov.policy_no = a.policy_no 
		where cov.last_inspection_dt is null
