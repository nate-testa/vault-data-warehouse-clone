	...
	BEGIN TRY
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				delete from edw_core.tcommercial_policy_summary
				where month_sk = @month_end_dt_sk; 

				DROP TABLE IF EXISTS edw_temp.tcommercial_policy_summary_can_rein_temp1;
				--insert cancels
				 SELECT policy_sk, policy_transaction_type_sk , transaction_seq_no 
				...
				and   expiration_dt_sk > @month_begin_dt_sk
				 group by policy_sk, policy_transaction_type_sk, transaction_seq_no 
				 union all
				 SELECT policy_sk, policy_transaction_type_sk , transaction_seq_no 
				 FROM	edw_core.tcommercial_policy_transaction
				...
				inner join ( select policy_sk, 
							...
							group by policy_sk) b on a.policy_sk = b.policy_sk
				...
				inner join (select policy_sk 
							from edw_temp.tcommercial_policy_summary_can_rein_temp1 
				            group by policy_sk
							having count(distinct policy_transaction_type_sk) > 1) c on a.policy_sk = c.policy_sk
				where not exists (select * from edw_temp.tcommercial_policy_summary_max_tr b where a.policy_sk = b.policy_sk and a.transaction_seq_no = b.transaction_seq_no);
				...
				with inf as
				(
				 SELECT policy_sk, premium_amt inforce_premium_amt
				 FROM	edw_core.tcommercial_daily_inforce_policy
				...
				(
					select distinct policy_sk,  
							min(calendar_month_sk) over (partition by policy_sk --order by transaction_seq_no
								) calendar_month_sk,
							max(transaction_seq_no) over (partition by policy_sk) max_tr_seq_no
					from edw_core.tcommercial_policy_transaction 
					where effective_dt_sk <= @end_dt_sk
					and   transaction_effective_dt_sk <= @end_dt_sk
					and   transaction_dt_sk <= @end_dt_sk  
				...
				( 
				 ...
				 FROM edw_core.tcommercial_policy_transaction tr
				 inner join edw_core.tcommercial_policy pol on tr.policy_sk = pol.policy_sk
				 left join edw_core.tcommercial_daily_inforce_policy inf on inf.policy_sk = pol.policy_sk and inf.inforce_dt_sk = @end_dt_sk
				 ...
				 and   tr.transaction_seq_no = (select min(tr1.transaction_seq_no) from edw_core.tcommercial_policy_transaction tr1 
				 								where tr1.policy_sk = tr.policy_sk)
				 and   tr.policy_transaction_sk = (select min(tr2.policy_transaction_sk) from edw_core.tcommercial_policy_transaction tr2 
				 								   where tr2.policy_sk = tr.policy_sk and tr2.transaction_seq_no = tr.transaction_seq_no) 
				),
				xpsr_exp as
				( 
				 ...
				 FROM edw_core.tcommercial_policy pol, edw_core.tcommercial_daily_inforce_policy inf
				 where inf.policy_sk = pol.policy_sk 
				   ...
				   and not exists (select policy_sk from edw_temp.tcommercial_policy_summary_can_rein_temp1 c where pol.policy_sk = c.policy_sk and c.policy_transaction_type_sk = 5)
				),
				xpsr_cancel as
				( 
					 select policy_sk , sum(we) we, sum(ee) ee
					 from
					 (
						 SELECT distinct tr.policy_sk, tr.transaction_seq_no,
		 						(
						 ...
						 FROM edw_core.tcommercial_policy_transaction tr
						 inner join edw_core.tcommercial_policy pol on tr.policy_sk = pol.policy_sk
						 left join min_tr on min_tr.policy_sk = pol.policy_sk  
						 --inner join min_tr max_tr on max_tr.policy_sk = tr.policy_sk and max_tr.max_tr_seq_no = tr.transaction_seq_no  
						 where exists (select policy_sk from edw_temp.tcommercial_policy_summary_can_rein_temp1 c where tr.policy_sk = c.policy_sk and tr.transaction_seq_no = c.transaction_seq_no and c.policy_transaction_type_sk = 5)
				...
				( 
					 ...
					 from
					 (
						 SELECT distinct tr.policy_sk, transaction_seq_no,
		 						(
						 ...
						 left join edw_core.tcommercial_daily_inforce_policy inf on inf.policy_sk = pol.policy_sk and inf.inforce_dt_sk = @end_dt_sk
						 --inner join min_tr max_tr on max_tr.policy_sk = tr.policy_sk and max_tr.max_tr_seq_no = tr.transaction_seq_no 
						 where exists (select policy_sk from edw_temp.tcommercial_policy_summary_can_rein_temp1 r where tr.policy_sk = r.policy_sk and tr.transaction_seq_no = r.transaction_seq_no and r.policy_transaction_type_sk = 6)
				...
				(
				 SELECT tr.policy_sk,  --tr.customer_sk, tr.broker_sk, tr.product_sk, pol.source_system_sk,
		 				...
		 				sum(case when tr.calendar_month_sk = @month_end_dt_sk THEN tr.tax_fee_surcharge_amt ELSE 0 END) mtd_tax_fee_surcharge_amt,
		 				sum(case when tr.calendar_month_sk = @month_end_dt_sk THEN tr.premium_amt - tr.tax_fee_surcharge_amt ELSE 0 END) mtd_net_premium_amt,
				 ...
				 FROM edw_core.tcommercial_policy_transaction tr, edw_core.tcommercial_policy pol 
				 where tr.policy_sk = pol.policy_sk --and pol.policy_sk in (107033)
				...
				INSERT INTO edw_core.tcommercial_policy_summary
					( 
						month_sk, policy_sk, policy_history_sk, customer_sk, broker_sk, product_sk, source_system_sk, 
				...
				left join xpsr_cancel on prm.policy_sk = xpsr_cancel.policy_sk
				left join xpsr_rein on prm.policy_sk = xpsr_rein.policy_sk
