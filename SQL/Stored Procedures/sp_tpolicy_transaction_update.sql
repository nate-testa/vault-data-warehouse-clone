...
-- 03/29/24		Architha Gudimalla				2. Added update for 0 veh_cov_sk, ssk = 2
-- 04/11/24		Architha Gudimalla				3. Added update for 0 veh_cov_sk, ssk = 4
-- ====================================================================================================================================================== 
   ...
   -- SET NOCOUNT ON added to prevent extra result sets from
	...
	BEGIN TRY
		...
		*/
		...
		-------------------------
		--au veh_cov_sk for ssk 4 - will only update for policies listed below, need to figure out why the issue is still happeneing for new pols with ssk = 4
		-------------------------
		...
		inner join (	
					Select distinct --pol.policy_sk, tr.transaction_seq_no, tr.item_sk , tr.internal_coverage_sk, 
						...
						from edw_temp.tpolicy_transaction_update_temp1 tr
						inner join edw_core.tpolicy pol on pol.policy_sk = tr.policy_sk
						left join edw_core.tauto_vehicle_coverage avc on pol.policy_no = avc.policy_no and pol.effective_dt = avc.effective_dt and tr.item_sk = avc.auto_vehicle_sk 
									and tr.transaction_seq_no >= avc.transaction_seq_no  
						where pol.policy_no+cast(pol.effective_dt as varchar)+cast(tr.transaction_seq_no as varchar)
									in  ('AU100037132-022023-04-015',
										 'AU100037132-022023-04-016',
										 'AU100091596-022023-11-152',
										 'AU100093013-022023-12-211',
										 'AU100109355-012023-01-182',
										 'AU100119990-032024-03-222',
										 'AU100123148-012023-03-218',
										 'AU100012333-022022-09-282')
					) tmp1 on tr.policy_transaction_sk = tmp1.policy_transaction_sk
		where product_sk = 3 and vehicle_coveragE_sk = 0 and tax_fee_surcharge_sk = 0
		and item_sk <> 0
		and source_system_sk = 4 ;
