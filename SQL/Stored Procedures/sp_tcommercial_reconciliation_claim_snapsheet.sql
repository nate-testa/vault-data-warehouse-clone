-- =================================================================================================
-- Description: This procedures provides total_loss_incurred differences between snapsheet and edw for commercial claims
---------------------------------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY		
        ...
        select @max_transaction_ts = max(transaction_ts) from edw_commercial.tcommercial_claim_transaction
        ...
        into edw_temp.sp_tcommercial_reconciliation_claim_snapsheet_temp1
        from
        (  select * from 
            (
                ...
                where ft.approved_at is not null and ft.created_at< = @max_transaction_ts
                    AND exists(select 1 from edw_stage_snapsheet.tags t where t.claim_id = c.id and [name]  like 'Commercial%')
        ...
        select c.commercial_claim_feature_sk,b.claim_no,c.claim_coverage_cd,
        SUM(
        ...
        where
        ct.commercial_claim_sk=b.commercial_claim_sk and ct.commercial_claim_feature_sk=c.commercial_claim_feature_sk
        group by c.commercial_claim_feature_sk,b.claim_no,c.claim_coverage_cd
        ...
        and b.commercial_claim_feature_sk=cf.commercial_claim_feature_sk
        and c.claim_no=cl.claim_number
        and cf.claim_coverage_cd=ex.id
        and cf.commercial_claim_feature_sk not in (
                                                                            select commercial_claim_feature_sk from edw_commercial.tcommercial_claim_feature 
                                                                            where (loss_reserve_amt<0 or expense_reserve_amt<0)
                                                                            )        
