...
-- Description: This stored procedure insert and update info related to tcommercial_policy_history.
-----------------------------------------------------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		INTO edw_temp.tcommercial_policy_history_temp3 --select acct.* 
		FROM edw_stage.AccountTransaction acct 
		INNER JOIN edw_stage.AccountTransactionVersion acctv ON acctv.AccountTransactionId = acct.Id  
		...
		INTO edw_temp.tcommercial_policy_history_temp2
		FROM
			(
				SELECT  acctv.AccountTransactionId, 
						acctvof.Field, 
						acctvof.Value
				FROM edw_temp.tcommercial_policy_history_temp1 acc
		...
		-- Pivot Table
		DROP TABLE IF EXISTS edw_temp.tcommercial_policy_history_temp4
		SELECT	
			 AccountTransactionId
			,[Rate Change] AS rate_change_pc
		INTO edw_temp.tcommercial_policy_history_temp4
		FROM
			(
				SELECT  
		...
		DROP TABLE IF EXISTS edw_temp.tcommercial_policy_history_temp5
		SELECT
			 source.PolicyNumber as policy_no
			...
			,cid.Name as transaction_issued_by
			,source.producer_sk
		...
		LEFT JOIN edw_temp.tcommercial_policy_history_temp2 source1 on source.id = source1.AccountTransactionId
		LEFT JOIN edw_temp.tcommercial_policy_history_temp4 tmp4 on tmp4.AccountTransactionId = source.id
	    left join edw_stage.[user] cid on source.createdbyid = cid.Id
	    left join edw_stage.[user] rid on source.reviewedbyid = rid.Id
		LEFT JOIN edw_commercial.tcommercial_policy pol on source.PolicyNumber = pol.policy_no and cast(source.EffectiveDate as date)=pol.effective_dt
		...
		INSERT INTO edw_commercial.tcommercial_policy_history
		(
			 policy_no
			,effective_dt
			,expiration_dt
		...
		where exists (select 'x' from edw_temp.tcommercial_policy_history_temp1 h1 where h.policy_no = h1.policynumber and h.effective_dt = cast(h1.EffectiveDate as date));

		update h
		set latest_transaction_in = 'Y'
		from edw_commercial.tcommercial_policy_history h
		where exists (select 'x' from edw_temp.tcommercial_policy_history_temp1 h1 
		...
		(
			...
			from edw_commercial.tcommercial_policy_history h
			where exists (select 'x' from edw_temp.tcommercial_policy_history_temp2 h1 where h.policy_no = h1.policynumber)
			group by policy_sk
		)
		update edw_commercial.tcommercial_policy_history  
		set latest_transaction_in = 'Y'
		from edw_commercial.tcommercial_policy_history h, max_tr 
