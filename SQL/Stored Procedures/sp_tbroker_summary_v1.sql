/****** Object:  StoredProcedure [edw_core].[sp_tbroker_summary_v1]    Script Date: 1/10/2024 3:09:53 AM ******/
SET ANSI_NULLS ON
GO
...
-- 01/05/24		Architha Gudimalla				10. Corrected prior year gross new prm
-- 01/06/24		Architha Gudimalla				11. Updated to use tclaim summary instead of feature summary 
-- 01/06/24		Architha Gudimalla				12. Corrected date filter for gross net prm
-- 01/06/24		Architha Gudimalla				13. Corrected date filter for current month mtd new prm
-- 01/09/24		Architha Gudimalla				14. Corrected non cat claim filter
-- 01/10/24		Architha Gudimalla				15. Used cast as date for quote dates to remove timestamp
-- 01/11/24		Architha Gudimalla				16. Added one and three year written prms
-- 02/29/24		Architha Gudimalla				17. Added one and three year non cat earned prms
-- 03/07/24		Architha Gudimalla				18. Added KPI calculations
-- 03/10/24		Architha Gudimalla				19. Updated KPI calculations to multiply by 100
-- 03/19/24		Architha Gudimalla				20. Exluded wind/hail from non cat prms 
-- 05/15/24		Architha Gudimalla				21. Removed from retention - expiring prm --> + isnull(r.expiring_non_renewal_written_premium_amt,0)
-- 05/15/24		Architha Gudimalla				22. Removed from retention - expiring ct --> -r.non_renewal_ct
-- 05/15/24		Architha Gudimalla				23. Added renewal offered count and premium - VI-30737
-- 05/21/24		Architha Gudimalla				24. Upong olivia'as request, Changed quote history join 
--													from	q.first_offered_quote_history_sk = qh.quote_history_sk 
--													to 		q.quote_sk = qh.quote_sk and qh.latest_transaction_in = 'Y' 
-- 06/12/24		Architha Gudimalla				25. Added
--													prior_mtd_net_premium_amt
--													mtd_net_premium_amt 
-- 07/18/24		Architha Gudimalla				26. Updated logic for @last_source_extract_ts
-- 07/26/24		Architha Gudimalla				27. Added changes for hubspot
-- 08/01/24		Architha Gudimalla				28. Added logic for policy_renewal_offered_over_50k_ct
-- 08/29/24		Architha Gudimalla				29. Corrected bind ct for hubspot
-- 10/13/24		Architha Gudimalla				30. Updated logic for policy_renewal_offered_ct
-- 10/13/24		Architha Gudimalla				30. Updated logic for policy_renewal_offered_over_50k_ct
-- 11/07/24		Architha Gudimalla				31. AD7643 | Updated retention logic, used mtd values for existing cols and added new ytd cols
--
--													Discussed with tyler, went over the calculations for retention % and retention prm % between brk and ren summ
--													ret from brk summ -- sum(policy_renewal_accepted_ct)/sum(policy_renewal_ct) 
--													ret from ren summ -- sum(renewal_non_flat_cancelled_ct)/sum(renewal_ct)
--													prm ret from brk summ --  sum(policy_renewal_premium_amt)/sum(policy_renewal_offered_expiring_premium_amt) 
--													prm ret from ren summ --  sum(renewal_sixty_day_written_premium_amt (where renewal_non_flat_cancelled_ct = 1))
--                                                                           /sum(expiring_written_premium_amt (where renewal_ct = 1)
--
--													any policy that is expiring and not flat cancelled, is termed as expring 
--													any policy that is renewed, whether or not it is cancelled, is termed as renewed
--													any renewal that is not flat cancelled is termed as renewal accepted
--													any policy that has a renewal quote offered is termed as renewal offered
-- 12/17/24		Architha Gudimalla				32. VI35593 - new columns request 
-- 02/13/25		Architha Gudimalla				33. AD8573 - excluded OS claims which do not have broker
-- 03/05/25		Hernando Gonzalez				34. AD8315 - Summary tables - Close Reasons
-- 10/02/25		Dinesh Bobbili					35. AD11217 - Added new columns
-- 10/07/25		Dinesh Bobbili					36. AD11217 - Updated the logics for non_flat_cancelled_ct, non_flat_cancelled_ct,expiring_sixty_day_written_premium_amt
-- 10/15/25		Dinesh Bobbili					37. AD11286 - Added date logic to run the process for 3 months
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @param_broker_sk INT
		select @param_broker_sk = broker_sk from edw_core.tbroker where broker_id = @in_broker_id;
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				--quotes data
				DROP TABLE IF EXISTS edw_temp.tbroker_summary_v1_quotes;
				select *
				into edw_temp.tbroker_summary_v1_quotes
				FROM
				(
					...
					left join edw_core.tquote_history qh on q.quote_sk = qh.quote_sk and qh.latest_transaction_in = 'Y' -- q.first_offered_quote_history_sk = qh.quote_history_sk 
					left join edw_core.tquote_home_coverage qhcov on qhcov.quote_history_sk = qh.quote_history_sk
					...
					and br.broker_sk = isnull(@param_broker_sk, br.broker_sk)
				...
				--retention data
				DROP TABLE IF EXISTS edw_temp.tbroker_summary_v1_retention;
				with ren AS
				(
					select 	r.month_sk,
					...
					left join edw_core.tquote_history qh  on q.quote_sk = qh.quote_sk and qh.latest_transaction_in = 'Y'
					where 	month_sk between @year_begin_dt_sk and @month_end_dt_sk  
					and 	r.non_flat_cancelled_ct = 1
					and r.broker_sk = isnull(@param_broker_sk, r.broker_sk)
				...
				into edw_temp.tbroker_summary_v1_retention
				from ren
				...
				*/ 
				...
				DROP TABLE IF EXISTS edw_temp.tbroker_summary_v1_pols;
				SELECT summ.broker_sk, summ.product_sk, summ.customer_sk, st.state_sk,
							...
							sum(case when summ.month_sk between     @month_begin_dt_sk and   @month_end_dt_sk then summ.mtd_net_premium_amt else 0 end)  mtd_written_net_premium_amt,
							sum(case when summ.month_sk between     @month_begin_dt_sk and   @month_end_dt_sk then summ.earned_premium_amt else 0 end)  mtd_earned_premium_amt,
							sum(case when summ.month_sk between     @prior_year_month_begin_dt_sk and @prior_year_month_end_dt_sk then summ.earned_premium_amt   else 0 end)  prior_mtd_earned_premium_amt,
							sum(case when summ.month_sk between     @month_begin_dt_sk and   @month_end_dt_sk then summ.earned_net_premium_amt else 0 end)  mtd_earned_net_premium_amt, 
							sum(case when summ.month_sk between     @prior_year_month_begin_dt_sk and @prior_year_month_end_dt_sk then summ.earned_net_premium_amt else 0 end)  prior_mtd_earned_net_premium_amt,   
							--
							...
							-- 
				 into edw_temp.tbroker_summary_v1_pols
				 ...
				 inner join edw_core.tpolicy pol on summ.policy_sk = pol.policy_sk
				 inner join edw_core.tstate st on pol.risk_state_cd = st.state_cd
				 inner join edw_core.tdate td on td.date_sk = summ.month_sk 
				 inner join edw_core.tpolicy_history ph_cancels on summ.policy_history_sk = ph_cancels.policy_history_sk 
				 inner join edw_core.tpolicy_history ph on summ.policy_sk = ph.policy_sk 
				 ...
				 and summ.broker_sk = isnull(@param_broker_sk, summ.broker_sk)
				 group by summ.broker_sk, summ.product_sk, summ.customer_sk, st.state_sk,
							replace(replace(isnull(pol.uw_company_nm,'Other'), 'Vault Reciprocal Exchange', 'VRE'), 'Vault E & S Insurance Company', 'VES')
				 
				--earned non cat net prms
				DROP TABLE IF EXISTS edw_temp.tbroker_summary_v1_int_cov_pols;
				SELECT summ.broker_sk, summ.product_sk, summ.customer_sk, st.state_sk,
							replace(replace(isnull(pol.uw_company_nm,'Other'), 'Vault Reciprocal Exchange', 'VRE'), 'Vault E & S Insurance Company', 'VES') uwco,    
							--
							sum(case when  summ.month_sk >       @prior_year_month_end_dt_sk then summ.earned_net_premium_amt else 0 end)   one_year_earned_net_premium_amt,
							...
							sum(case when  summ.month_sk > @prior_year_five_month_end_dt_sk then summ.earned_net_premium_amt else 0 end) five_year_earned_net_premium_amt
							--
				 into edw_temp.tbroker_summary_v1_int_cov_pols
				 FROM	edw_core.tinternal_coverage_summary summ, edw_core.tpolicy pol, edw_core.tstate st,  edw_core.tdate td,
				 		edw_core.tpolicy_history ph, edw_core.tinternal_coverage ic,
				...
				--inforce and ytd prm data
				DROP TABLE IF EXISTS edw_temp.tbroker_summary_v1_pols1;
				SELECT ph.broker_sk, ph.product_sk, ph.customer_sk, st.state_sk,
							...
							--
				into edw_temp.tbroker_summary_v1_pols1
				FROM	edw_core.tpolicy pol
				inner join  edw_core.tpolicy_history ph on pol.policy_sk = ph.policy_sk
				inner join (select policy_sk, min(transaction_seq_no) transaction_seq_no
							from edw_core.tpolicy_history
				...
				where ph.broker_sk = isnull(@param_broker_sk, ph.broker_sk)
				group by ph.broker_sk, ph.product_sk, ph.customer_sk, st.state_sk,
						replace(replace(isnull(pol.uw_company_nm,'Other'), 'Vault Reciprocal Exchange', 'VRE'), 'Vault E & S Insurance Company', 'VES')
				
				--claims date
				DROP TABLE IF EXISTS edw_temp.tbroker_summary_v1_claims;
				select  cs.broker_sk, cs.product_sk, cs.customer_sk, st.state_sk,
							replace(replace(isnull(pol.uw_company_nm,'Other'), 'Vault Reciprocal Exchange', 'VRE'), 'Vault E & S Insurance Company', 'VES') uwco, 
							--
							sum(case when c.loss_dt between dateadd(mm,-1,@end_dt) and @end_dt then 1 else 0 end) mtd_claim_ct, 
							sum(case when c.loss_dt between dateadd(mm,-1,dateadd(yy,-1,@end_dt)) and dateadd(yy,-1,@end_dt) then 1 else 0 end) prior_mtd_claim_ct, 
							sum(case when c.loss_dt between dateadd(yy,-1,@end_dt) and @end_dt then 1 else 0 end) one_year_claim_ct, 
							sum(case when c.loss_dt between dateadd(mm,-1,@end_dt) and @end_dt then [itd_total_incurred_amt] else 0 end) mtd_loss_incurred_amt,
							sum(case when c.loss_dt between dateadd(mm,-1,dateadd(yy,-1,@end_dt)) and dateadd(yy,-1,@end_dt) then [itd_total_incurred_amt] else 0 end) prior_mtd_loss_incurred_amt,
							...
							sum(case when c.loss_dt between dateadd(mm,-1,@end_dt) and @end_dt 
									 then (case when [itd_total_incurred_amt] > 500000 then 500000 else [itd_total_incurred_amt] end) 
									 else 0 end
							) mtd_loss_incurred_capped_amt, 
							sum(case when c.loss_dt between dateadd(mm,-1,dateadd(yy,-1,@end_dt)) and dateadd(yy,-1,@end_dt)
									 then (case when [itd_total_incurred_amt] > 500000 then 500000 else [itd_total_incurred_amt] end) 
									 else 0 end
							) prior_mtd_loss_incurred_capped_amt, 
							sum(case when c.loss_dt between dateadd(yy,-1,@end_dt) and @end_dt 
									 then (case when [itd_total_incurred_amt] > 500000 then 500000 else [itd_total_incurred_amt] end) 
									 else 0 end
							) one_year_loss_incurred_capped_amt, 
							sum(case when c.loss_dt between dateadd(mm,-1,@end_dt) and @end_dt and c.catastrophe_sk is null then 1 else 0 end) mtd_non_cat_claim_ct, 
							sum(case when c.loss_dt between dateadd(mm,-1,dateadd(yy,-1,@end_dt)) and dateadd(yy,-1,@end_dt) and c.catastrophe_sk is null then 1 else 0 end) prior_mtd_non_cat_claim_ct, 
							...
							sum(case when c.loss_dt between dateadd(mm,-1,@end_dt) and @end_dt and c.catastrophe_sk is null 
									 then [itd_total_incurred_amt] 
									 else 0 
							end) mtd_non_cat_loss_incurred_amt,
							sum(case when c.loss_dt between dateadd(mm,-1,dateadd(yy,-1,@end_dt)) and dateadd(yy,-1,@end_dt) and c.catastrophe_sk is null 
									 then [itd_total_incurred_amt] 
									 else 0 
							end) prior_mtd_non_cat_loss_incurred_amt,  
							...
							) mtd_non_cat_loss_incurred_capped_amt, 
							sum(case when c.loss_dt between dateadd(mm,-1,dateadd(yy,-1,@end_dt)) and dateadd(yy,-1,@end_dt) and c.catastrophe_sk is null
									 then (case when [itd_total_incurred_amt] > 500000 then 500000 else [itd_total_incurred_amt] end) 
									 else 0 end
							) prior_mtd_non_cat_loss_incurred_capped_amt, 
							sum(case when c.loss_dt between dateadd(yy,-1,@end_dt) and @end_dt and c.catastrophe_sk is null
									 then (case when [itd_total_incurred_amt] > 500000 then 500000 else [itd_total_incurred_amt] end) 
									 else 0 end
					...
					into edw_temp.tbroker_summary_v1_claims
					from edw_core.tclaim_summary cs, edw_core.tclaim c,  edw_core.tpolicy pol, edw_core.tstate st 
					where	cs.claim_sk  = c.claim_sk
					and 	cs.policy_sk = pol.policy_sk 
					 and 	pol.risk_state_cd = st.state_cd
					and 	month_sk in (@month_end_dt_sk)
					and cs.broker_sk is not null
					and cs.broker_sk = isnull(@param_broker_sk, cs.broker_sk)
					group by cs.broker_sk, cs.product_sk, cs.customer_sk, st.state_sk,
							replace(replace(isnull(pol.uw_company_nm,'Other'), 'Vault Reciprocal Exchange', 'VRE'), 'Vault E & S Insurance Company', 'VES')
				
				DROP TABLE IF EXISTS edw_temp.tbroker_summary_v1_temp;
				with pol_summ as
				(
					select * 
					from  edw_temp.tbroker_summary_v1_pols
					 
				), pol_summ1 as
				(
					select * 
					from  edw_temp.tbroker_summary_v1_pols1
					 
				),  pol_summ2 as
				(
					select * 
					from  edw_temp.tbroker_summary_v1_int_cov_pols
					 
				), 
				clm_summ as
				( 
					select *
					from edw_temp.tbroker_summary_v1_claims  
				) , 
				quotes as
				(   
					select *
					from edw_temp.tbroker_summary_v1_quotes
				)   
				select 	COALESCE(ps.broker_sk, 		ps1.broker_sk, 		ps2.broker_sk, 		cs.broker_sk, 	q.broker_sk) broker_sk,
						...
						--
						sum(isnull(ps.mtd_written_net_premium_amt,0)) mtd_written_net_premium_amt,
						sum(isnull(ps.mtd_earned_premium_amt,0)) mtd_earned_premium_amt,
						sum(isnull(ps.prior_mtd_earned_premium_amt,0)) prior_mtd_earned_premium_amt,
						sum(isnull(ps.mtd_earned_net_premium_amt,0)) mtd_earned_net_premium_amt,
						sum(isnull(ps.prior_mtd_earned_net_premium_amt,0)) prior_mtd_earned_net_premium_amt,
						...
						--
						sum(isnull(cs.mtd_claim_ct,0)) mtd_claim_ct,
						sum(isnull(cs.prior_mtd_claim_ct,0)) prior_mtd_claim_ct,
						sum(isnull(cs.one_year_claim_ct,0)) one_year_claim_ct,
						sum(isnull(cs.mtd_loss_incurred_amt,0)) mtd_loss_incurred_amt,
						sum(isnull(cs.prior_mtd_loss_incurred_amt,0)) prior_mtd_loss_incurred_amt,
						sum(isnull(cs.one_year_loss_incurred_amt,0)) one_year_loss_incurred_amt,
						sum(isnull(cs.mtd_loss_incurred_capped_amt,0)) mtd_loss_incurred_capped_amt,
						sum(isnull(cs.prior_mtd_loss_incurred_capped_amt,0)) prior_mtd_loss_incurred_capped_amt,
						sum(isnull(cs.one_year_loss_incurred_capped_amt,0)) one_year_loss_incurred_capped_amt,
						sum(isnull(cs.mtd_non_cat_claim_ct,0)) mtd_non_cat_claim_ct,
						sum(isnull(cs.prior_mtd_non_cat_claim_ct,0)) prior_mtd_non_cat_claim_ct,
						sum(isnull(cs.one_year_non_cat_claim_ct,0)) one_year_non_cat_claim_ct,
						sum(isnull(cs.mtd_non_cat_loss_incurred_amt,0)) mtd_non_cat_loss_incurred_amt,
						sum(isnull(cs.prior_mtd_non_cat_loss_incurred_amt,0)) prior_mtd_non_cat_loss_incurred_amt,
						sum(isnull(cs.one_year_non_cat_loss_incurred_amt,0)) one_year_non_cat_loss_incurred_amt,
						sum(isnull(cs.mtd_non_cat_loss_incurred_capped_amt,0)) mtd_non_cat_loss_incurred_capped_amt,
						sum(isnull(cs.prior_mtd_non_cat_loss_incurred_capped_amt,0)) prior_mtd_non_cat_loss_incurred_capped_amt,
				...
				into edw_temp.tbroker_summary_v1_temp
				from pol_summ ps
				...
				delete from edw_stage.tbroker_summary_v1
				where month_sk = @month_end_dt_sk
				and broker_sk = isnull(@param_broker_sk, broker_sk); 
       
				SET @rows_affected=0;

				INSERT INTO edw_stage.tbroker_summary_v1
					( 
						month_sk,
						broker_sk,
						product_sk, 
						...
						inforce_tiv_amt, 
						--
						mtd_written_net_premium_amt,
						mtd_earned_premium_amt,
						prior_mtd_earned_premium_amt,
						mtd_earned_net_premium_amt,
						prior_mtd_earned_net_premium_amt,
						...
						prior_mtd_claim_ct,
						one_year_claim_ct,
						mtd_loss_incurred_amt,
						prior_mtd_loss_incurred_amt,
						one_year_loss_incurred_amt,
						mtd_loss_incurred_capped_amt,
						prior_mtd_loss_incurred_capped_amt,
						one_year_loss_incurred_capped_amt,
						mtd_non_cat_claim_ct,
						prior_mtd_non_cat_claim_ct,
						one_year_non_cat_claim_ct,
						mtd_non_cat_loss_incurred_amt,
						prior_mtd_non_cat_loss_incurred_amt,
				...
				select @month_end_dt_sk, 
						...
						--
						sum(summ.mtd_written_net_premium_amt),
						sum(summ.mtd_earned_premium_amt),
						sum(summ.prior_mtd_earned_premium_amt),
						sum(summ.mtd_earned_net_premium_amt),
						sum(summ.prior_mtd_earned_net_premium_amt),
						...
						--
						sum(summ.mtd_claim_ct),
						sum(summ.prior_mtd_claim_ct),
						sum(summ.one_year_claim_ct),
						sum(summ.mtd_loss_incurred_amt),
						sum(summ.prior_mtd_loss_incurred_amt),
						sum(summ.one_year_loss_incurred_amt),
						sum(summ.mtd_loss_incurred_capped_amt),
						sum(summ.prior_mtd_loss_incurred_capped_amt),
						sum(summ.one_year_loss_incurred_capped_amt),
						sum(summ.mtd_non_cat_claim_ct),
						sum(summ.prior_mtd_non_cat_claim_ct),
						sum(summ.one_year_non_cat_claim_ct),
						sum(summ.mtd_non_cat_loss_incurred_amt),
						sum(summ.prior_mtd_non_cat_loss_incurred_amt),
						sum(summ.one_year_non_cat_loss_incurred_amt),
						sum(summ.mtd_non_cat_loss_incurred_capped_amt),
						sum(summ.prior_mtd_non_cat_loss_incurred_capped_amt),
						...
						--
				from edw_temp.tbroker_summary_v1_temp summ
				LEFT join edw_temp.tbroker_summary_v1_retention r on summ.broker_sk = r.broker_sk and summ.product_sk = r.product_sk and summ.customer_sk = r.customer_sk and summ.uwco = r.uwco and summ.state_sk = r.state_sk  
				group by  
						coalesce(summ.broker_sk, r.broker_sk), 
						coalesce(summ.product_sk, r.product_sk), 
						coalesce(summ.customer_sk, r.customer_sk), 
				...
				UPDATE edw_stage.tbroker_summary_v1
				set				
