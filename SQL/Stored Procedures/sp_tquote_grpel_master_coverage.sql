...
CREATE OR ALTER PROCEDURE [edw_core].[sp_tquote_grpel_master_coverage]
...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS edw_temp.tquote_grpel_master_coverage_temp1;
		DROP TABLE IF EXISTS edw_temp.tquote_grpel_master_coverage_temp2;		
		...
		into edw_temp.tquote_grpel_master_coverage_temp1
		from
		(
			select 
				acct.Id,acct.PolicyNumber,acct.EffectiveDate,acct.[Number],acct.ExpirationDate,acct.CreatedDate,
		...
		into edw_temp.tquote_grpel_master_coverage_temp2
		from
		(
		select 
		acct.Id,
		...
		acctvof.[Value]
		from
			edw_temp.tquote_grpel_master_coverage_temp1 acct
			--inner join [edw_stage].[Account] as acc on act.AccountId = acc.id
			inner join edw_stage.AccountTransactionVersion acctv on acct.Id=acctv.AccountTransactionId
			inner join edw_stage.AccountTransactionVersionObject acctvo on acctv.Id=acctvo.AccountTransactionVersionId 
		...
		INSERT INTO [edw_core].[tquote_grpel_master_coverage]
		(
		...
		select
			...
			a.[Number] as transaction_seq_no,
		...
		from
			edw_temp.tquote_grpel_master_coverage_temp1 a
			left join edw_temp.tquote_grpel_master_coverage_temp2 b on a.Id = b.Id

		SET @rows_affected=@@ROWCOUNT;
	
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(t2.CreatedDate) FROM edw_temp.tquote_grpel_master_coverage_temp1 t2),@last_source_extract_ts);
