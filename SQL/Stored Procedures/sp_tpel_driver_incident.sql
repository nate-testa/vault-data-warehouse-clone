...
BEGIN
	...
	BEGIN TRY
		...
		from
			...
			CAST(act.TransactionEffectiveDate AS DATE) AS TransactionEffectiveDate,atvo.[Index],tph.policy_history_sk,
		...
		INSERT INTO [edw_core].[tpel_driver_incident]
		(
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(IssuedDate) FROM edw_temp.tpel_driver_incident_temp1),@last_source_extract_ts)	
