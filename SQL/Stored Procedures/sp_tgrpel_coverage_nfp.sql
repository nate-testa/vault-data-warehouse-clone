...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
					...
					SUM(np.nfp_program_sharing_funding) OVER (
						PARTITION BY 
							np.insured_cert_no,
							np.term_effective_date,
							np.transaction_seq_no
					) AS nfp_program_sharing_funding_amt,
					
					SUM(np.vault_fronting_fees_xol_cc_collected) OVER (
						PARTITION BY 
							np.insured_cert_no,
							np.term_effective_date,
							np.transaction_seq_no
					) AS vault_fronting_fees_excessofloss_ceding_comm_collected_amt,
					
					SUM(np.qs_cc_collected) OVER (
						PARTITION BY 
							np.insured_cert_no,
							np.term_effective_date,
							np.transaction_seq_no
					) AS quota_share_ceding_comm_collected_amt,
					
					SUM(np.total_cc_collected) OVER (
						PARTITION BY 
							np.insured_cert_no,
							np.term_effective_date,
							np.transaction_seq_no
					) AS total_ceding_comm_collected_amt,
					
					SUM(np.vault_cc_override_after_expenses) OVER (
						PARTITION BY 
							np.insured_cert_no,
							np.term_effective_date,
							np.transaction_seq_no
					) AS vault_ceding_comm_override_after_expenses_amt,
					
					SUM(np.vault_fronting_fees) OVER (
						PARTITION BY 
							np.insured_cert_no,
							np.term_effective_date,
							np.transaction_seq_no
					) AS vault_fronting_fees_amt,
					
					SUM(np.add_fee_income_due) OVER (
						PARTITION BY 
							np.insured_cert_no,
							np.term_effective_date,
							np.transaction_seq_no
					) AS add_fee_income_due_amt,
					
					SUM(np.vault_prem_w_profitshare_frontfees) OVER (
						PARTITION BY 
							np.insured_cert_no,
							np.term_effective_date,
							np.transaction_seq_no
					) AS vault_premium_with_profitshare_frontfees_amt,
					
					cast(np.risk_group as varchar(255)) AS risk_group,
					
					SUM(np.fronting_fee_total) OVER (
						PARTITION BY 
							np.insured_cert_no,
							np.term_effective_date,
							np.transaction_seq_no
					) AS fronting_fee_total_amt,
		...
		SELECT np.policy_no                                                 
			...
			,np.employment_practises_liability_premium_amt                
			,np.non_profit_do_liability_limit_amt                         
			,np.non_profit_do_liability_premium_amt                       
			,np.family_trust_management_liability_limit_amt               
			,np.family_trust_management_liability_premium_amt             
			,np.no_of_residences                                          
			,np.no_of_vehicles                                            
			,np.no_of_drivers_under_22                                    
			,np.no_of_drivers_from_22_to_75                               
			,np.no_of_drivers_76_and_older                                
			,np.underlying_auto_liability_limit_amt                       
			,np.underlying_home_liability_limit_amt                       
			,np.underlying_watercraft_liability_limit_amt                 
			,np.underinsured_liability_limit_desc                                                                           
			,np.nfp_program_sharing_funding_amt                           
			,np.vault_fronting_fees_excessofloss_ceding_comm_collected_amt
			,np.quota_share_ceding_comm_collected_amt                     
			,np.total_ceding_comm_collected_amt                           
			,np.vault_ceding_comm_override_after_expenses_amt             
			,np.vault_fronting_fees_amt                                   
			,np.add_fee_income_due_amt                                    
			,np.vault_premium_with_profitshare_frontfees_amt              
			,np.risk_group                                                
			,np.fronting_fee_total_amt                                    
			,np.underwriting_year_pc                                      
			,np.reporting_month
			,np.dup_rn
		INTO edw_temp.tgrpel_coverage_temp1
		FROM nfp_base np
		inner join edw_core.tpolicy_history tph
			on np.policy_no = tph.policy_no
			and np.effective_dt = tph.effective_dt
		...
		SELECT 
			...
			,@etl_audit_sk
		FROM 
			edw_temp.tgrpel_coverage_temp1

		SET @rows_affected=@@ROWCOUNT;

		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(t1.reporting_month) FROM edw_temp.tgrpel_coverage_temp1 t1),@last_source_extract_ts);
