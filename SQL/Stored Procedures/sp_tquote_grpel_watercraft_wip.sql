...
-- Description: This procedures insert grpel watercraft data for wip quotes
---------------------------------------------------------------------------------------------------------------------
-- Change date 			|Author							| Change Description
---------------------------------------------------------------------------------------------------------------------
-- 03/10/26				Yunus Mohammed					1. Procedure created
-- ==================================================================================================================
CREATE OR ALTER PROCEDURE [edw_core].[sp_tquote_grpel_watercraft_wip]
...
BEGIN
	...
	BEGIN TRY
		...
		drop table if exists edw_temp.tquote_grpel_watercraft_wip_temp1
		select 
			...
			CreatedDate,UpdatedDate,[Year],Make,Model,watercraft_unique_id
		into edw_temp.tquote_grpel_watercraft_wip_temp1
		from
		(
		select * 
		from
		...
		pivot 
		(
			max([Value]) FOR Field IN ([Year],Make,Model,[Length]
        )
		) as pivottable

		MERGE INTO [edw_core].[tquote_grpel_watercraft] AS TARGET
		USING (
		    SELECT
		    ...
		    FROM
		        edw_temp.tquote_grpel_watercraft_wip_temp1 AS ttpv
		) AS SOURCE
		ON
		...
		WHEN NOT MATCHED BY TARGET THEN
		    ...
		    VALUES (
		        ...
		        SOURCE.watercraft_no, SOURCE.watercraft_year, SOURCE.watercraft_make, SOURCE.watercraft_model,Source.watercraft_unique_id,
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(greatest(UpdatedDate,CreatedDate)) FROM edw_temp.tquote_grpel_watercraft_wip_temp1),@last_source_extract_ts)	
