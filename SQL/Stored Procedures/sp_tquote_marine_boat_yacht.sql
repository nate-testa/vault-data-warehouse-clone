...
-- Description: This stored procedure insert and update info related to tquote_marine_boat_yacht
---------------------------------------------------------------------------------------------------------------------------------------
-- Change date          |Author						|	Change Description
---------------------------------------------------------------------------------------------------------------------------------------
-- 11/22/24     		Yunus Mohammed			    1. Created the procedure
-- ====================================================================================================================================
CREATE OR ALTER  PROCEDURE [edw_core].[sp_tquote_marine_boat_yacht]
AS
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS [edw_temp].[tquote_marine_boat_yacht_temp1];

        SELECT *
        INTO [edw_temp].[tquote_marine_boat_yacht_temp1]
        FROM
        (
        SELECT
        ...
        FROM
            (
                SELECT
                    acct.PolicyNumber, acct.EffectiveDate, acct.ExpirationDate, acct.CreatedDate, acct.Number,
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(CreatedDate) FROM edw_temp.[tquote_marine_boat_yacht_temp1]),@last_source_extract_ts);
