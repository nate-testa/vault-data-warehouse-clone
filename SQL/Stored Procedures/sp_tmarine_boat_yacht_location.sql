...
-- ================================================================================================= 
CREATE OR ALTER PROCEDURE [edw_core].[sp_tmarine_boat_yacht_location]
...
BEGIN
	...
	BEGIN TRY
		...
		drop table if exists edw_temp.tmarine_boat_yacht_location_temp1
		select 
		...
		INSERT INTO [edw_core].[tmarine_boat_yacht_location]
		(
		...
		FROM
			edw_temp.tmarine_boat_yacht_location_temp1 AS ttlc

		SET @rows_affected=@@ROWCOUNT;

		-- Update control table
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(IssuedDate) FROM edw_temp.tmarine_boat_yacht_location_temp1),@last_source_extract_ts)	
