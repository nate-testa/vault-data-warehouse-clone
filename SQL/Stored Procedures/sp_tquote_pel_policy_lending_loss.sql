...
GO
-- ================================================================================================================================================
-- Description: This stored procedure insert and update info related to tquote_pel_policy_lending_loss.
--------------------------------------------------------------------------------------------------------------------------------------------------
...
--
-- ================================================================================================================================================
CREATE OR ALTER PROCEDURE [edw_core].[sp_tquote_pel_policy_lending_loss] 
AS
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS [edw_temp].[tquote_pel_policy_lending_loss_temp1];
		
        SELECT CreatedDate,quote_no,effective_dt,transaction_effective_dt,expiration_dt,transaction_dt,
        ...
        INTO [edw_temp].[tquote_pel_policy_lending_loss_temp1]
        FROM (
            SELECT  
            acct.CreatedDate, acct.PolicyNumber as quote_no, acct.EffectiveDate as effective_dt, acct.TransactionEffectiveDate as transaction_effective_dt, 
                        acct.ExpirationDate as expiration_dt, acct.IssuedDate as transaction_dt, acct.Number as transaction_seq_no,
                        qh.quote_history_sk,
		...
		INSERT INTO [edw_core].[tquote_pel_policy_lending_loss]
        (
            quote_no
            ,effective_dt
            ,expiration_dt
        ...
        FROM 
            [edw_temp].[tquote_pel_policy_lending_loss_temp1] AS t1
        ;
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(CreatedDate) FROM edw_temp.[tquote_pel_policy_lending_loss_temp1]),@last_source_extract_ts);
