...
CREATE OR ALTER PROCEDURE [edw_temp].[sp_tpolicy_history_backfill_tktno]
...
BEGIN
	...
	BEGIN TRY
		...
		(
			...
			WHERE
			rn=1
		) [source] ON [target].claim_no = [source].claim_no

		SET @rows_affected=@@ROWCOUNT;
        DROP TABLE IF EXISTS edw_temp.tpolicy_history_backfill_tktno_temp1
        DROP TABLE IF EXISTS edw_temp.tpolicy_history_backfill_tktno_temp2
