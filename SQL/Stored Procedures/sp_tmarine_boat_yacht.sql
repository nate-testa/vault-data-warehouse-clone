...
-- Description: This stored procedure insert and update info related to tmarine_boat_yacht
---------------------------------------------------------------------------------------------------------------------------------------
-- Change date          |Author						|	Change Description
---------------------------------------------------------------------------------------------------------------------------------------
-- 11/22/24     		Yunus Mohammed			    1. Created the procedure
-- ====================================================================================================================================
CREATE OR ALTER  PROCEDURE [edw_core].[sp_tmarine_boat_yacht]
AS
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF EXISTS [edw_temp].[tmarine_boat_yacht_temp1];

        SELECT *
        INTO [edw_temp].[tmarine_boat_yacht_temp1]
        FROM
        (
        SELECT
        ...
        FROM
            (
                SELECT
                    acct.PolicyNumber, acct.EffectiveDate, acct.ExpirationDate, acct.IssuedDate, acct.PolicyChangeNumber,
                ...
                WHERE
                    ...
                    AND acctvo.[ObjectType] = 'BoatYacht'
            ) t
        PIVOT 
            (
		...
		-- Start Merge process
		MERGE [edw_core].[tmarine_boat_yacht] AS [target]
		USING [edw_temp].[tmarine_boat_yacht_temp1] AS [source]
		ON [source].policy_no = target.policy_no AND [source].effective_dt = target.effective_dt      
		-- For Inserts
		WHEN NOT MATCHED BY TARGET THEN
		INSERT (
            policy_no,effective_dt,expiration_dt,boat_yatch_product_type,boat_yatch_mep,boat_yacht_make,
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(IssuedDate) FROM edw_temp.[tmarine_boat_yacht_temp1]),@last_source_extract_ts);
