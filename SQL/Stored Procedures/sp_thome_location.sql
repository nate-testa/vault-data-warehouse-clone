...
BEGIN
	...
	BEGIN TRY
		...
		DROP TABLE IF exists edw_temp.thome_location_temp1

		SELECT * INTO [edw_temp].[thome_location_temp1]
		FROM (SELECT
				acct.*,
				CASE WHEN acct.ExternalSourceId IS NOT NULL THEN 2 ELSE 4 END source_system_sk,
				DENSE_RANK() OVER (PARTITION BY acct.PolicyNumber, CAST(acct.EffectiveDate AS DATE) ORDER BY acct.policychangenumber DESC) AS policy_txn_order
		...
		-- Pivot Table
		DROP TABLE IF EXISTS [edw_temp].[thome_location_temp2];
		...
		FROM
			(
			SELECT 
				DENSE_RANK()OVER(PARTITION BY act.PolicyNumber,CAST(act.EffectiveDate AS DATE) ORDER BY act.policychangenumber DESC) AS policy_txn_order,
			...
			act.policychangenumber AS transaction_seq_no,act.source_system_sk, act.IssuedDate as transactiondate,act.IssuedDate,
			atvof.Field,atvof.[Value]
			from
				[edw_temp].[thome_location_temp1] act
		...
		-- Start Merge process
		MERGE [edw_core].[thome_location] AS Target
		USING edw_temp.thome_location_temp2 AS Source
		ON Source.PolicyNumber = Target.[policy_no]
		and Source.EffectiveDate=Target.effective_dt
		-- For Inserts
		WHEN NOT MATCHED BY Target THEN
		INSERT (
				policy_no,effective_dt,address_line_1,address_line_2,unit_no,city_nm,
		...
		SET @new_last_source_extract_ts=COALESCE((SELECT MAX(IssuedDate) FROM edw_temp.thome_location_temp1),@last_source_extract_ts)
